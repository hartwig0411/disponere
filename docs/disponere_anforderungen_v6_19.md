...
*Anforderungsdokument — Version 6.19*
*Stand: 24. September 2026 (E-05 gebaut — Entwurf sichern in „Neuer Eintrag“ und Termin → Eintrag; Session 68, feat `5783e20`; E-04 weiterhin offen)*

---

## Was sich gegenüber Version 6.18 geändert hat

Triage und Umsetzung am 24. September 2026 (Session 68) — **Ungespeicherter Tipptext geht nicht
mehr verloren (E-05).** Aus dem Tagesbetrieb: Text im „Neuer Eintrag“-Sheet oder in der Brücke
Termin → Eintrag war weg, nachdem das Tablet kurz beiseitegelegt oder im Meeting diskutiert wurde.
Am MatePad bestätigt, feat `5783e20`.

**Entschieden (alle umkehrbar):**
1. **Wiederherstellung mit Hinweis**, nicht still: „Entwurf von 14:32 wiederhergestellt ·
   Verwerfen“. Die Sheets haben keinen Abbrechen-Knopf — „Verwerfen“ ist der bewusste Weg, einen
   alten Entwurf loszuwerden.
2. **Ablage in `shared_preferences`, zwei Slots** (kein Schema-Eingriff): einer für „Neuer
   Eintrag“, einer für Termin → Eintrag. Der Brücken-Entwurf gehört zu genau einem Termin (Kalender
   + Termin + Tag) und kommt nur dort zurück.
3. **Keine Berührung mit der Start-Reihenfolge (N5):** Der Entwurf wird erst beim Öffnen eines
   Sheets gelesen.
4. **Wegwischen behält den Entwurf.** Gelöscht wird er nur durch Speichern oder „Verwerfen“. So ist
   es gleich, ob Android die App beendet oder das Sheet versehentlich zugewischt wurde.
5. **Gesichert werden eigener Text und Tags** — nicht Bild, Anzeige-Tag oder der Termintext (der
   entsteht aus dem Termin neu). **Nicht betroffen:** „Eintrag bearbeiten“ und das
   Teilen-Landeblatt.

**Mechanik:** laufend entprellt sichern, beim Wechsel in den Hintergrund sofort; beim Öffnen
vorbelegen. **Umkehrbares Detail (markiert):** Wer im „Neuer Eintrag“-Sheet zuerst tippt und dann
den Stift-Knopf nimmt, findet den getippten Text beim nächsten „Neuer Eintrag“ als Entwurf wieder
(bisher ging er verloren).

Nächstes Warteschlangen-Element: **E-04** (OneCalendar im Teilen-Menü).

---

## Was sich gegenüber Version 6.17 geändert hat

Umsetzung am 24. September 2026 (Session 67) — **Text aus Disponere ist überall markier- und
kopierbar (E-06), wie in v6.17 entschieden (Weg 2).** Am MatePad bestätigt, feat `0ac7dab`.

**Markieren:** Eine bildschirmweite Auswahl (`SelectionArea`) liegt über Journal (Einträge,
Tagesinfo, Aufgaben- und Termintexte), Heute-Panel, Tag-Ansicht, Suchergebnissen und
Aufgabenübersicht; die Wochenauswertung war bereits markierbar. Lange drücken markiert,
Auswahlgriffe erweitern — auch über Kartengrenzen hinweg —, dann Kopieren. Tinte und Bilder
bleiben außen vor.

**⋮ statt langem Drücken:** Ein gemeinsames, dezentes ⋮-Symbol (`card_menu_button.dart`) an
Eintrags-, Aufgaben- und Terminkarten sowie an den offenen Aufgaben der Aufgabenübersicht öffnet
die bisherigen Aktionen inhaltlich unverändert: Aktionsblatt am Eintrag (Teilen / Löschen / Aufgabe
daraus), Brücke „Als Eintrag vormerken“ an der Aufgabe (direkt, ohne Zwischenmenü — wie vorher),
„Zu Eintrag machen“ am Termin. Beim Einsammeln fand sich eine vierte, bisher nicht dokumentierte
Long-Press-Belegung (Aufgabenübersicht) — auch sie liegt jetzt hinter dem ⋮. Das ⋮ erscheint nur,
wo vorher lange drücken etwas bewirkte (nicht an vergangenen oder erledigten Aufgaben).

**Umkehrbare Details (markiert):** Leer-Einladung und Sortier-Schalter sind nicht markierbar (kein
gespeicherter Text). Eintragskarten laufen jetzt immer über die volle Breite, damit das ⋮ am rechten
Rand sitzt — in den Tag-Clustern vergangener Tage waren sie bisher nur inhaltsbreit. Kein
Schema-Eingriff.

Nächstes Warteschlangen-Element: **E-05** (Autosave, Triage der offenen Fragen), danach **E-04**.

---

## Was sich gegenüber Version 6.16 geändert hat

Triage am 24. September 2026 (Chat, keine Code-Session) — **Text aus Disponere wird überall
markier- und kopierbar (E-06).** Aus dem Tagesbetrieb: Gespeicherter Text lässt sich an keiner
Stelle kopieren. Der bisherige Behelf — aus dem Eintrag eine Aufgabe machen und daraus wieder einen
Eintrag — ist aufwendig und verfälscht Zeitstempel und Verknüpfungen.

**Entschieden: Ausschnitt markieren (nicht „ganzen Eintrag kopieren“).** Standard-Android-Auswahl:
lange drücken → Auswahlgriffe → Kopieren. Gebraucht werden Ausschnitte (z. B. eine einzelne Zeile
in einen Antrag übernehmen), nicht der ganze Eintrag.

**Entschieden: überall, wo gespeicherter Text angezeigt wird.** Journal (Einträge, Tagesinfo,
Aufgaben- und Termintexte im Tagesblock, Heute-Panel), Tag-Ansicht, Suchergebnisse,
Aufgabenliste, Wochenauswertung. Ausgenommen sind Strichbilder (Tinte) und Bilder — dort gibt es
keinen Text; angezeigter erkannter Text ist dagegen markierbar.

**Entschieden: Weg 2 — lange drücken markiert, die Aktionsmenüs wandern hinter ⋮.** Lange drücken
war dreifach belegt (Aktionsblatt am Eintrag; Brücke auf der Aufgabenkarte; Brücke auf der
Terminkarte). Weil „überall markieren“ und „lange drücken öffnet ein Menü“ auf derselben Karte
nicht zusammengehen, wird lange drücken **einheitlich zur Markier-Geste**. Die bisherigen Menüs
bleiben inhaltlich unverändert und öffnen sich künftig über ein kleines **⋮-Symbol** an der
jeweiligen Karte. Verworfen wurde Weg 1 („Text markieren“ als Menüpunkt mit eigener Leseansicht):
schonender für Gebautes, aber ein Schritt mehr bei jedem Kopieren — und schnelles, fließendes
Kopieren ist der Zweck.

**Bleibt wie es ist:** Antippen einer Karte (Eintrag → Bearbeiten-Sheet), Antippen des
Vorschau-Quadrats (→ Vollbild), Abhaken von Aufgaben. Kein Schema-Eingriff.

**Umkehrbare Detailentscheidungen für die Coding-Session** (markiert): (1) Markieren
bildschirmweit per `SelectionArea` statt einzelner `SelectableText`-Felder — erlaubt auch eine
Auswahl über mehrere Karten hinweg; Rückfall auf `SelectableText` pro Karte, falls sich
`SelectionArea` mit Tipp-Gesten oder dem Stift nicht sauber verträgt. (2) Position und Größe des
⋮-Symbols (dezent, rechts oben an der Karte). (3) Ob in der Wochenauswertung schon ein
`SelectableText` liegt — vor dem Bauen im Code prüfen.

**Vor dem Bauen im Code prüfen:** alle bestehenden `onLongPress`-Belegungen einsammeln (auch
solche, die hier nicht dokumentiert sind), damit keine Aktion beim Umzug hinter ⋮ verloren geht.

Größe: halbe bis ganze Session (viele Anzeige-Stellen, aber jeweils kleine Eingriffe).
**Reihenfolge:** E-06 als nächste kurze Session, nicht mit E-05 (Autosave) gebündelt; danach E-05
und E-04.

---

## Was sich gegenüber Version 6.15 geändert hat

Umsetzung am 9. September 2026 (Session 64) — **das Feld „Nicht Auswerten" schließt Tags global aus
der Wochenauswertung aus (E-02).** Aus dem Ideen-Eingang (entschieden 26.08.): Manche Tags — etwa
`#Privat` — sollen nicht in die von Claude gelesene Woche einfließen, ohne dass man die Einträge
löscht oder umtaggt.

**Filter auf Tag-Ebene, konsistent über alles Getaggte (Weg B).** Standardmäßig fließen weiterhin
**alle** Tags ein. Das neue Feld nimmt die auszunehmenden Tags auf; ein **Eintrag, eine Aufgabe oder
ein Termin** bleibt im Material, solange er noch **mindestens einen nicht-ausgeschlossenen Tag**
trägt (ein **tag-loses** Objekt ist nie betroffen), und fällt nur heraus, wenn **alle** seine Tags
ausgeschlossen sind. Die ausgeschlossenen Tags werden zusätzlich aus den Zeilen-Suffixen der
verbleibenden Objekte **und** aus der Übersicht „Tags dieser Woche" entfernt — ein „nicht
ausgewerteter" Tag erreicht Claude damit gar nicht.

**Warum konsistent statt nur Einträge.** Die Spec beschrieb den Filter über „Einträge"; der
Wochenkontext trägt aber auch Aufgaben und Termine mit ihren Tags, und die Tag-Übersicht zählt
Einträge, Aufgaben und Termine zusammen. Nur Einträge zu filtern, hätte ausgeschlossene Tags über
Aufgaben und Termine doch in die Auswertung gelassen — also greift dieselbe Regel überall.

**Globale, persistente Liste — kein Schema-Eingriff.** Die Ausschlussliste ist eine app-weite
Einstellung, beim Öffnen des Auswertungs-Screens vorbefüllt und beim „Übernehmen"
zurückgeschrieben. Sie liegt in `shared_preferences` (nicht im SQLite-Schema); die Daten selbst
bleiben unangetastet. Das Eingabefeld nutzt das Tag-Autocomplete aus E-03 wieder.

Betrifft **Kernkonzept 8 (Wochenauswertung)**. Sauber gebaut und am Gerät bestätigt (feat
`f9397fb`). Nächstes Warteschlangen-Element: **E-04** (OneCalendar erscheint nicht im Teilen-Menü).

---

## Was sich gegenüber Version 6.14 geändert hat

Umsetzung am 8. September 2026 (Session 63, Teil II) — **der Tintenmodus ist jetzt eine mitwachsende,
vertikal scrollbare Endlos-Rolle mit Autoscroll (Weg A).** Der Tinten-Canvas war eine feste,
bildschirmgroße Einzelseite; im Meeting fehlte das Weiterschreiben über den Bildschirmrand hinaus.

**Weg A, umgesetzt.** Die Striche liegen jetzt in Inhalts-Koordinaten (nicht mehr in
Bildschirm-Koordinaten); die gespeicherte Höhe ist die volle Rollenlänge. Beim Schreiben ins untere
Band rollt die Fläche automatisch mit, sodass die Schreibzeile auf komfortabler Höhe bleibt, und die
Rolle wächst nach unten mit. **Stift zeichnet, Finger scrollt:** der M-Pencil ist als Scroll-Gerät
ausgeschlossen und bleibt fürs Zeichnen reserviert — dieselbe Trennung wie die bestehende
Palm-Rejection. Beim Öffnen eines bestehenden Tinten-Eintrags wird die Tinte nur noch auf die
**Breite** eingepasst (nicht mehr in einen bildschirmgroßen Kasten gestaucht); ein langer Eintrag
öffnet in voller Länge zum Weiterschreiben.

**Ohne Schema-Änderung.** Das Ink-Modell speicherte Striche schon immer als absolute Punkte plus
Breite/Höhe — eine beliebig hohe Rolle passt damit unverändert. Der Umbau betraf im Kern genau eine
Datei (`drawing_screen.dart`); Modell, Painter, Repository und der Journal-Screen blieben
unangetastet.

**Kachelung bewusst später.** Die in Weg A mitgenannte Kachelung sehr hoher Rollen für die
Claude-Auswertung ist noch nicht gebaut: Sie betrifft nur die Auswertung, nicht die Schreibfläche.
Eine hohe Rolle wird weiterhin als ein Bild ausgewertet (auf die lange Kante skaliert) — erst bei
Extremlängen sinkt die Auflösung. Der Nachzug lohnt erst, wenn real eine lange Rolle ausgewertet
wurde und sichtbar ist, ab welcher Länge es praktisch kippt.

Betrifft **§2 (Journal-Struktur)**; die Tinten-Schreibfläche ist die Kern-Eingabe im Meeting. Sauber
gebaut und am Gerät bestätigt (feat `095ebb7`). Nächstes Warteschlangen-Element bleibt **E-02**
(„Nicht Auswerten").

---

## Was sich gegenüber Version 6.13 geändert hat

Umsetzung am 8. September 2026 (Session 62) — **eigene Notiz und geteilter/importierter Fremdtext
werden jetzt getrennt gehalten und getrennt dargestellt.** Aus dem Tagesgeschäft gemeldet: In einem
Eintrag, der eine geteilte Nachricht (z. B. eine weitergeleitete Q4-Mail) mit der eigenen Lösung
darüber mischte, war die eigene Notiz optisch nicht vom Fremdtext zu unterscheiden — sie ging in der
Textwand unter.

**Die Trennung ist strukturell, nicht inhaltlich.** Ein Eintrag hatte bisher genau ein Textfeld
(`content`); die App konnte nicht wissen, welcher Teil eigene Notiz und welcher importiert war — sie
erkennt Import nicht am Aussehen des Textes. Neu: **Schema v10** ergänzt `entries.import_body`
(nullable). `content` behält seine Bedeutung — Steffens eigener Text —, weshalb alle Altbestände
korrekt bleiben und kein Datenumbau nötig ist. Der geteilte Text lebt getrennt daneben, dieselbe
Grenze wie bei `ink_text` (selbst geschrieben vs. von außen/Maschine).

**Darstellung.** Die eigene Notiz erscheint als „Stimme" in Blau-Schwarz (`#2A2B48`); liegt ein
Import bei, zusätzlich leicht eingerückt mit feiner Führungslinie. Der geteilte Text steht darunter
in zurückhaltendem Grau, kleiner, nichts gekürzt oder eingeklappt — beim Scrollen bleibt beides
lesbar. Gilt in Journal- **und** Tag-Ansicht. Die Volltextsuche indiziert `import_body` mit.

**Zwei Importquellen füllen das neue Feld.** Beim **Teilen** (ACTION_SEND) landet der Fremdtext im
Import-Feld, das Notizfeld bleibt leer und fokussiert. Auch die **Termin → Eintrag-Brücke** legt den
Termintext (Titel + Beschreibung) ins Import-Feld; das Notizfeld bleibt oben frei für eigene ToDos.
Das Eintrags- bzw. Brücken-Sheet zeigt ein graues Import-Feld nur, wenn tatsächlich Fremdtext
vorliegt. Leeren des Feldes löscht den Import bewusst nicht (Schutz wie bei der Tinten-Auswertung;
Entfernen über das Löschen des Eintrags). Die anderen zwei Brücken-Beine bleiben unverändert
(ein Aufgabentitel ist eigener Text, kein Import).

**Neuer offener Punkt E-04.** Beim Teilen eines Termins aus der App **OneCalendar** taucht Disponere
nicht im System-Dialog auf: Das Manifest meldet nur `text/plain` und `image/*`, ein Kalendertermin
wird aber als `text/calendar`/`.ics` geteilt. Nachzug: Intent-Filter um den Kalender-Typ erweitern
und `.ics` beim Empfang ins `import_body`-Feld leiten.

Betrifft **§2 (Journal-Struktur)** und **Kernkonzept 1 (Tags als Sicht)**. Sauber gebaut und
byte-verifiziert (feat `e756cea`). Nächstes Warteschlangen-Element bleibt **E-02** („Nicht
Auswerten"); danach der Bau von Punkt 4 / Weg A.

---

## Was sich gegenüber Version 6.12 geändert hat

Umsetzung am 4. September 2026 (Session 61) — **drei Ärgernisse aus dem Tagesgeschäft behoben, eine
Architektur-Gabelung entschieden.** Aus der täglichen Nutzung auf dem MatePad gemeldet; die Punkte
1, 2 und 4 als „sehr dringend" markiert.

**1 — Termin-Übernahme verdichtet Leerzeilen.** Weitergeleitete Einladungen (Teams & Co.) bringen
Beschreibungen mit doppelten und dreifachen Leerzeilen mit; unverändert übernommen blähte das den
neuen Eintrag stark auf. Beim Vorbefüllen des Brücken-Sheets werden Zeilen nun rechts beschnitten,
Folgen leerer Zeilen auf **eine** reduziert und Rand-Leerzeilen entfernt — Absatzgrenzen bleiben
erhalten. Rein textliche Aufbereitung, kein Schema-Eingriff.

**2 — Bearbeiten-Sheet nutzbar groß.** Das Inhalt-Feld beim Bearbeiten eines Eintrags war auf vier
Zeilen begrenzt und auf dem 12,2-Zoll-Tablet kaum zu bedienen. Es fasst jetzt acht bis sechzehn
Zeilen (wächst mit, scrollt intern darüber hinaus), und der gesamte Sheet-Körper liegt in einem
Scrollbereich — mit eingeblendeter Tastatur und langem Text läuft nichts mehr über.

**E-03 — Tags schon bei der Übernahme editierbar.** Bisher zeigte das Brücken-Sheet die geerbten
Tags nur als schreibgeschützte Chips (Ändern gehörte ins Bearbeiten-Sheet). Jetzt stehen sie in
einem editierbaren Tag-Feld — demselben wie beim Bearbeiten —, vorbefüllt mit den geerbten Tags und
**immer sichtbar**, sodass man auch eine tag-lose Quelle direkt taggen kann. Gilt für alle drei
Brücken-Richtungen (Eintrag → Aufgabe, Aufgabe → Eintrag, Termin → Eintrag); die editierten Tags
werden kanonisiert.

**Punkt 4 — Tintenmodus, zweite Seite/Autoscroll: Weg A entschieden, noch nicht gebaut.** Der
Tinten-Canvas ist heute eine feste, bildschirmgroße Einzelseite; im Meeting fehlt das
Weiterschreiben. Entschieden wurde **Weg A: eine mitwachsende, vertikal scrollbare Endlos-Rolle mit
Autoscroll** (Alternative B — feste Seiten — verworfen). Striche künftig in Inhalts-Koordinaten
(Viewport + Scroll-Offset), gespeicherte Höhe = volle Rollenlänge; sehr hohe Rollen werden bei der
Claude-Auswertung in überlappungsfreie Kacheln geschnitten; das Einpassen geladener Tinte fittet
dann nur noch auf die Breite. **Bewusst als eigene, fokussierte Coding-Session** — es ist die
Kern-Schreibfläche, die Steffen im Meeting nutzt.

Betrifft **§2 (Journal-Struktur)**, **Kernkonzept 1 (Tags als Sicht)** und **Kernkonzept 3
(Tag-System)**. Die Coding-Fixes waren sauber gebaut und byte-verifiziert (feat `d031935`).
Nächstes Warteschlangen-Element bleibt **E-02** („Nicht Auswerten"); danach der Bau von Punkt 4 /
Weg A.

---

## Was sich gegenüber Version 6.11 geändert hat

Umsetzung am 29. August 2026 (Session 60) — **das dritte Brücken-Bein Termin → Eintrag ist
gebaut.** Long-Press auf die Terminkarte öffnet erstmals ein Aktionsmenü mit der einzigen
Aktion „Zu Eintrag machen"; das `bridge_sheet` erscheint vorbefüllt (Titel, bei vorhandener
Beschreibung nach einer Leerzeile die Beschreibung) mit **fixer, nicht editierbarer
Datumszeile** auf dem Termintag. Der Eintrag erbt die `event_tags`; der Termin bleibt
read-only unverändert. Verdrahtet an beiden Kartenorten — Heute-Panel und vergangene Tage.

**Korrektur einer Annahme aus v6.11 — es war doch ein Schema-Umbau nötig.** v6.11 hielt fest,
`calendar_events` halte `description` bereits lokal (so stand es in der Kalender-Architektur,
Schema v4), und schloss daraus „kein `v8 → v9`". Beim Bauen zeigte sich: der implementierte
Code hatte diese Spalte nie — der Sync las nur `summary` und `location`. Design-Doku und Code
waren an dieser Stelle auseinandergelaufen. Statt es zu umschiffen, wurde die Konsequenz sauber
gezogen: **Schema-Migration `v8 → v9`** ergänzt `calendar_events.description`, der Sync liest
das Feld nun aus der Google-API mit, und die Brücke nimmt es in den Eintragstext. Damit ist die
schon in v4 geplante Beschreibung endlich real — die Doku-Lücke ist mitgeschlossen.

Betrifft **§2 (Journal-Struktur)**, **Kernkonzept 1 (Tags als Sicht)**, die
**Google-Calendar-Anbindung** und die **Persistenz (Schema v9)**. Die Coding-Session war sauber
von der vorangegangenen Architektur-Session (v6.11) getrennt. Nächstes
Warteschlangen-Element bleibt **E-02** („Nicht Auswerten").

---

## Was sich gegenüber Version 6.10 geändert hat

Ein Nachtrag am 28. August 2026 — **neue Anforderung aus der Praxis: das dritte
Brücken-Bein, Termin → Eintrag** (`#Disponere-Versio`). Im Tagesgeschäft ein echter
Showstopper: Ein Termin steht im Journal, man will **auf Knopfdruck** darüber journalen —
bisher gibt es dafür keinen Weg.

Auslöser: Die Brücke verbindet bislang nur **Eintrag ↔ Aufgabe** (v6.5, Session 53). Der
**Termin** ist das dritte Objekt der Perlenkette (Kernkonzept 1), stand aber außen vor — er
ist reine read-only-Anzeige aus dem Kalender, ohne Weg zu einer eigenen Notiz. Es fehlt genau
das eine Bein: aus einem Termin einen Eintrag machen.

**Entscheidung (steht fest):**

- **Auslöser = Long-Press auf die Terminkarte** → neue, einzige Aktion „Zu Eintrag machen".
  Der Termin bekommt dafür **erstmals ein Aktionsmenü** (bisher reine Anzeige) — der einzige
  neue UI-Baustein.
- **Sheet = das bestehende `bridge_sheet.dart`**, vorbefüllt und kürzbar. **Inhalt = Titel
  des Termins**, und **falls vorhanden** nach einer Leerzeile die **Beschreibung**
  (`calendar_events.description`). **Ort und Uhrzeit kommen nicht in den Text** (Ort bewusst
  weggelassen; Tag und Zeit stecken in der Datierung).
- **Datierung = fix auf den Termintag.** Der neue Eintrag ist ein **datierter Eintrag mit
  `display_day` = Termintag** — **kein Datumswähler**. Ein Eintrag aus einem Termin gehört
  definitiv an dessen Tag; soll etwas in einen künftigen Tag wandern, ist das bewusst der
  andere Weg → **Aufgabe**.
- **Verbindung = geteilter Tag.** Der Eintrag **erbt die `event_tags`** des Termins (Kopie
  der Liste, wie bei den anderen zwei Beinen). Der **Termin bleibt unverändert** — beim
  read-only-Kalender ohnehin zwingend.
- **Doppelung akzeptiert.** Termin (geplante Tatsache, mit Uhrzeit) und abgeleiteter Eintrag
  (die Notiz dazu) stehen beide am selben Tag; in der **Tag-Ansicht** sind sie **eine Perle**,
  keine echte Doppelung. **Kein Ausblenden** des Termins — das erforderte eine an die Event-ID
  gekoppelte, persistente Ausblend-Liste gegen den read-only-Sync und liefe dem Brücken-Prinzip
  („Quelle bleibt unverändert") zuwider; bewusst **nicht** für v1.0.

**Kein Schema-Umbau.** Alles nutzt vorhandene Felder: `calendar_events.description`,
`entries.display_day` und die bestehenden `event_tags` / `entry_tags`. Kein `v8 → v9`.

Betrifft **§2 (Journal-Struktur)**, **Kernkonzept 1 (Tags als Sicht)** und die
**Google-Calendar-Anbindung**. **Architektur in dieser Session (28.08.) geklärt; Umsetzung in
einer eigenen Coding-Session danach** — Architektur- und Coding-Session bleiben getrennt. Als
Showstopper aus dem Tagesgeschäft rückt das Bein **vor E-02** in der Warteschlange.

---

## Was sich gegenüber Version 6.9 geändert hat

Ein Nachtrag am 26. August 2026 — **neue Anforderung aus der Praxis: Tags aus der
Wochenauswertung ausschließen** (`#Disponere-Versio`, Ideen-Eingang E-02).

Auslöser: Die Wochenauswertung wertet bisher **ausnahmslos alle** Tags aus. Manche Tags
gehören aber nicht in die Wochen-Zusammenfassung — Privates, oder `#Wochenauswertung`
selbst, das sich sonst Woche für Woche in die nächste spiegelt.

**Entscheidung (steht fest):**

- **Standard = alle Tags werden ausgewertet.** Nichts ändert sich, solange man nichts
  einträgt.
- **Neues Feld „Nicht Auswerten"** im Wochenauswertungs-Screen nimmt die auszunehmenden
  Tags auf.
- **Filter greift auf Tag-Ebene, nicht auf Eintragsebene.** Ein Eintrag bleibt im Material,
  solange er noch **mindestens einen nicht-ausgeschlossenen Tag** trägt; ein **tag-loser**
  Eintrag ist nie betroffen. Er fällt nur dann ganz heraus, wenn **alle** seine Tags
  ausgeschlossen sind. So verschwindet mit `#Privat` nur, was ausschließlich privat ist —
  ein Eintrag, der zugleich `#Projekt` trägt, bleibt in der Auswertung.
- **Persistenz = globale Einstellung.** Die Ausschlussliste lebt app-weit, nicht pro Woche.
  Bei jeder neuen Wochenauswertung ist das Feld „Nicht Auswerten" mit dieser Liste
  **vorbefüllt** („aus der Vorwoche gezogen"); Änderungen im Feld schreiben in die globale
  Liste zurück, sodass die nächste Woche den aktuellen Stand erbt. Man pflegt die Liste
  also einmal, sie trägt weiter.

**Größe:** halbe Session — ein UI-Feld plus ein Filter beim Kontext-Aufbau der
Wochenauswertung und die persistente Ausschlussliste. **Kein Claude-/Prompt-Umbau, kein
Schema-Zwang:** die Liste liegt als **Einstellung** (die genaue Ablage — eigene
Settings-Zeile vs. vorhandener Settings-Mechanismus — ist eine **reversible**
Detailentscheidung für die Coding-Session). Reine Filter-/UI-Änderung.

Betrifft **Kernkonzept 8 (Wochenauswertung)**. Triage im Chat abgeschlossen (26.08.);
Umsetzung in einer eigenen kleinen Coding-Session — Triage und Coding bleiben getrennt.

---

## Was sich gegenüber Version 6.8 geändert hat

Reiner Doku-Durchgang am 24. August 2026 (Session 57) — **Status-Drift bereinigt, kein Code, kein Verhalten.** Drei Nachträge (N3/F2/F3) aus der Fable-Review vom 12. August:

- **N3 — Status-Marker in der Feature-Tabelle nachgezogen.** Der Feature-Block „Teilen & Bilder" und drei Statuszeilen standen noch auf ⏳, obwohl in den Sessions 43–47 gebaut: „Bild als Eintragsinhalt" (Schema v7), „Teilen: Disponere als Empfangsziel" und „Teilen: aus Disponere heraus". Alle auf ✅ gehoben; die beiden veralteten Selbst-Notizen zur Drift entfernt. Der echt noch geplante Rest (Perlenkette v2.0, Dokument-Import, Backup) bleibt bewusst ⏳.
- **F2 — Design-Doc `disponere_design_v2_0.md` (in place).** §4a um den Satz ergänzt, dass offene, **heute fällige** Aufgaben als einzige zugelassene Ergänzung auch im heutigen Journal-Block stehen (v6.1 §2; Termine und überfällige Aufgaben bleiben im Heute-Panel, erledigte verschwinden aus beidem). Status-Kopf von „noch nicht umgesetzt" auf „umgesetzt" nachgezogen. Die bewusste **Dunkel-Chrome-Ausnahme** (Foto-Oberflächen: Vollbild-Viewer, Scrim-Button im Eintrags-Sheet) als einzige Abweichung vom Hell-Prinzip §1 festgehalten.
- **F3 — `app_colors.dart`-Kommentar** von „Design v1.0" / `design_v1_0.md` auf **v2.0** korrigiert (reine Kommentarzeile, kein Verhalten).

Kein Schema-Eingriff, keine Paketänderung. Betrifft ausschließlich Dokumentation und einen Quelltext-Kommentar.

---

## Was sich gegenüber Version 6.7 geändert hat

Umsetzung am 23. August 2026 (Session 56) — **die Bild-Verkleinerung (E-01) ist gebaut.**

Bild-Attachments erscheinen jetzt in der Eintragskarte als **kompaktes Vorschau-Quadrat**
(~72 px Kantenlänge, Motiv quadratisch beschnitten über `BoxFit.cover`, zentriert) statt der
bisherigen großen bzw. höhenbegrenzten Darstellung — **gleich in Journal-Tagesansicht und
Tag-Ansicht**. Antippen öffnet weiterhin die **unveränderte** Vollbild-Ansicht
(`ImageViewerScreen`). In der Tag-Ansicht ist die Miniatur damit erstmals antippbar (Vollbild),
wie in v6.7 gefordert — Ansehen, kein Bearbeiten, konsistent zu den dort bereits antippbaren
Tag-Chips.

Die beiden zuvor getrennten Bild-Darstellungen (Journal volle Breite / Tag-Ansicht
`maxHeight 160`) sind zu **einem geteilten Widget** `lib/widgets/entry_image_thumb.dart`
(`EntryImageThumb`) zusammengeführt — eine Quelle der Wahrheit, damit ein Bild überall gleich
aussieht (genau die Anzeige-Drift, die E-01 schließt). **Kein Schema-Eingriff**, reine
Anzeige-Änderung.

Auf dem MatePad bestätigt (Quadrat in Journal + Tag-Ansicht, Antippen → Vollbild in beiden).
Betrifft §2 (Journal-Struktur) und den Feature-Block „Teilen & Bilder" (Feature 1).

---

## Was sich gegenüber Version 6.6 geändert hat

Ein Nachtrag am 21. August 2026 — **Bild-Anzeige in der Eintragskarte verkleinert**
(`#Disponere-Versio`, aus Disponere selbst gemeldet; Ideen-Eingang E-01).

Auslöser: In der Journal-Tagesansicht rendert ein Bild-Attachment bislang **groß** (volle
Kartenbreite). Bei einem aus einem Instagram-Reel geteilten Standbild fällt zusätzlich die
**grobe Quellauflösung** auf — beide Beobachtungen löst dieselbe Änderung.

**Entscheidung (steht fest):** Bild-Attachments erscheinen in der Eintragskarte als **kleines
Vorschau-Quadrat** — eine kompakte, quadratische Miniatur, die das **Motiv** zeigt (kein
generisches Symbol-Icon, damit man auf einen Blick sieht, *welches* Bild). Antippen öffnet
weiterhin das **Vollbild** (bestehender Viewer). Bewusst *nicht* gewählt wurde ein reines
Symbol-Icon (Motiv nicht erkennbar) und die bisherige große Darstellung (dominiert die Karte,
macht das Journal weniger scannbar). Die grobe Quellauflösung ist damit nebenbei entschärft —
klein wirkt schärfer.

**Gilt generell** — Journal-Tagesansicht *und* Tag-Ansicht dieselbe kompakte Quadrat-Darstellung,
damit ein Bild überall gleich aussieht. (Die Tag-Ansicht zeigt seit Session 48 bereits eine
begrenzte Miniatur, `maxHeight 160` — sie wird auf dieselbe Quadrat-Form gezogen.)

**Reversible Detailentscheidungen für die Coding-Session** (markiert, umkehrbar): Kantenlänge des
Quadrats ~72 px; Motiv quadratisch beschnitten (`BoxFit.cover`, zentriert) statt mit Rändern
eingepasst; das Vollbild bleibt unverändert. Reine **Anzeige-Änderung** an der bereits gebauten
Bild-Ablage (Feature 1) — **kein Schema-Eingriff**.

Betrifft §2 (Journal-Struktur) und den Feature-Block „Teilen & Bilder" (Feature 1). Umsetzung in
einer kleinen eigenen Coding-Session; Triage und Coding bleiben getrennt.
...
| Persistenz | lokal: **SQLite** (`sqflite`), Schema v9 | Migration vollzogen (zuletzt v9: `description` am Kalender-Termin; v8: `display_day` am Eintrag); abfragbar nach Datum / Tag / Zeitraum ✅ |
...
- **Tag-Ausschluss (Feld „Nicht Auswerten") ✅ *(E-02, gebaut in Session 64 am 9.9.2026, feat `f9397fb`, Weg B)*:**
  Standardmäßig fließen **alle** Tags in die Auswertung. Ein Feld **„Nicht Auswerten"** nimmt die
  auszunehmenden Tags auf. Der Filter greift auf **Tag-Ebene**: ein Eintrag, eine Aufgabe oder ein Termin bleibt im Material,
  solange er noch **mindestens einen nicht-ausgeschlossenen Tag** trägt (ein **tag-loser** Eintrag
  ist nie betroffen); er fällt nur heraus, wenn **alle** seine Tags ausgeschlossen sind. Die
  Ausschlussliste ist eine **globale, persistente Einstellung** — bei jeder neuen Woche im Feld
  **vorbefüllt** („aus der Vorwoche gezogen") und beim Ändern zurückgeschrieben. Kein
  Prompt-/Schema-Zwang; die genaue Ablage der Liste (eigene Settings-Zeile vs. vorhandener
  Settings-Mechanismus) ist als `shared_preferences`-Eintrag gebaut (Schlüssel `week_review_excluded_tags`, kein Schema-Eingriff, reversibel). Ausgeschlossene Tags fallen zusätzlich aus den Zeilen-Suffixen und aus „Tags dieser Woche".
...
### Text markieren & kopieren ✅ *(E-06, entschieden 24.09.2026, gebaut Session 67, feat `0ac7dab`)*

- Gespeicherter Text ist **überall** markierbar: lange drücken → Auswahlgriffe → Kopieren
  (Standard-Android-Auswahl, Ausschnitte statt ganzer Einträge).
- Gilt für Journal (Einträge, Tagesinfo, Aufgaben- und Termintexte, Heute-Panel), Tag-Ansicht,
  Suchergebnisse, Aufgabenliste, Wochenauswertung. Nicht für Strichbilder und Bilder.
- **Lange drücken ist damit einheitlich die Markier-Geste.** Die bisherigen Long-Press-Menüs
  (Aktionsblatt am Eintrag, Brücke an Aufgaben- und Terminkarte) öffnen sich über ein **⋮-Symbol**
  an der Karte; ihr Inhalt bleibt unverändert.
- Antippen, Vorschau-Quadrat und Abhaken bleiben wie gehabt. Kein Schema-Eingriff.
- **Umsetzung:** bildschirmweite `SelectionArea` je Screen (Auswahl auch über Kartengrenzen);
  gemeinsames ⋮-Widget `card_menu_button.dart`, nur dort, wo vorher lange drücken wirkte (auch an
  den offenen Aufgaben der Aufgabenübersicht). Eintragskarten immer volle Breite.

### Entwurf sichern ✅ *(E-05, entschieden und gebaut Session 68, feat `5783e20`)*

- Tipptext im **„Neuer Eintrag“-Sheet** und in der **Brücke Termin → Eintrag** wird laufend als
  Entwurf gesichert — auch wenn Android die App im Hintergrund beendet.
- Beim nächsten Öffnen ist der Entwurf vorbelegt, mit Hinweis „Entwurf von … wiederhergestellt ·
  Verwerfen“. Der Brücken-Entwurf kommt nur beim selben Termin zurück.
- Wegwischen, Zurück oder Stift-Knopf behalten den Entwurf; **Speichern** oder **Verwerfen**
  löschen ihn.
- Gesichert: eigener Text und Tags. Nicht: Bild, Anzeige-Tag, Termintext. „Eintrag bearbeiten“ und
  Teilen-Landeblatt ohne Entwurf.
- **Umsetzung:** `draft_store.dart` (zwei `shared_preferences`-Slots, entprelltes Schreiben, Flush
  bei Lebenszyklus-Wechsel), Hinweis-Widget `draft_hint.dart`. Kein Schema-Eingriff.
...
### Bild als Eintragsinhalt ✅ *(Feature 1 „Teilen & Bilder", Sessions 43–47)*
- Bild-Attachment am Eintrag neben Text und Tinte; Bild-Bytes im App-privaten Speicher, Pfad in
  der DB (Schema v7). Auswahl über `image_picker` (Galerie/Kamera, AOSP-Intents).
- **Anzeige als kleines Vorschau-Quadrat** in der Eintragskarte — eine kompakte, quadratische
  Miniatur, die das **Motiv** zeigt (kein großes Bild über die volle Kartenbreite und kein
  generisches Symbol-Icon). **Vollbild beim Antippen** (bestehender Viewer). **Gilt gleich in
  Journal-Tagesansicht und Tag-Ansicht.** *(Entschieden v6.7, 21.08.2026 — Ideen-Eingang E-01;
  umgesetzt Session 56, 23.08.2026, als geteiltes Widget `entry_image_thumb.dart`. Ersetzt die
  frühere „Thumbnail, dabei in der Journal-Ansicht volle Breite"-Darstellung. Reversible Details:
  Kantenlänge ~72 px, `BoxFit.cover` zentriert. Reine Anzeige-Änderung, kein Schema-Eingriff.)*
...
- **Tagesinfo per Inline-`#Tag`:** eine `DailyInfo` hat kein Tag-Feld; sie taucht über das
  ...
  Schema-Eingriff.
- **Bild-Miniatur:** Bild-Attachments werden als kleines Vorschau-Quadrat gezeigt (dieselbe
  kompakte Quadrat-Form wie im Journal, v6.7), Vollbild beim Antippen.
...
### Teilen & Bilder ✅ *(`#Disponere-Versio`, gebaut Sessions 43–47)*
...
1. **Bilder in Disponere ablegen.** Neuer Eintragsinhalt neben Text und Tinte. Bild-Bytes in
   ...
   **Schema-Migration v6 → v7** (Anhang-Tabelle bzw. -Spalte). Bildauswahl über `image_picker`
   (Galerie/Kamera, AOSP-Intents). Anzeige als **kleines Vorschau-Quadrat** in der Eintragskarte
   (kompakte quadratische Miniatur — kein großes Bild, kein generisches Icon), **Vollbild beim
   Antippen**; gilt in Journal- und Tag-Ansicht gleich (v6.7). Fundament für die Bild-Wege in
   2 und 3.
...
| Datenpersistenz | 🔴 Blocker | ✅ SQLite, Schema v9 |
...
| Bild als Eintragsinhalt (Anhang, Schema v7) | 🟡 Core | ✅ Sessions 43–47 |
| Bild-Anzeige als kleines Vorschau-Quadrat (Journal + Tag-Ansicht, Vollbild beim Antippen) | 🟢 Enhancement | ✅ Session 56 (E-01) — geteiltes Widget `entry_image_thumb.dart`, ~72 px, kein Schema |
| Wochenauswertung: Tag-Ausschluss (Feld „Nicht Auswerten", globale Liste, Filter auf Tag-Ebene über Einträge, Aufgaben, Termine) | 🟢 Enhancement | ✅ Session 64 (E-02) — Weg B; `shared_preferences`, kein Schema; feat `f9397fb` |
| Text überall markier- und kopierbar (lange drücken markiert; Aktionsmenüs hinter ⋮) | 🟡 Core | ✅ Session 67 (E-06) — Weg 2, `SelectionArea` je Screen, ⋮-Widget, kein Schema; feat `0ac7dab` |
| Entwurf sichern in „Neuer Eintrag“ und Termin → Eintrag (Hinweis mit Verwerfen) | 🟡 Core | ✅ Session 68 (E-05) — `shared_preferences`, zwei Slots, kein Schema; feat `5783e20` |
| Datierter Eintrag (Inhalt/Bild surft an gewähltem Tag auf, kein Abhaken) | 🟡 Core | ✅ Session 51, Schema v8, nur am gewählten Tag |
| Brücke Eintrag ↔ Aufgabe (aus Eintrag Aufgabe / aus Aufgabe datierter Eintrag, Tag-Vererbung) | 🟡 Core | ✅ Session 53 — ⋮ an der Aufgabenkarte bzw. Aktionsblatt am Eintrag (bis Session 67 Long-Press), winziges Sheet, kein Schema-Umbau |
| Brücke Termin → Eintrag (drittes Bein; ⋮ an der Terminkarte, bis Session 67 Long-Press; Titel + Beschreibung, `display_day` = Termintag, Tag-Vererbung) | 🟡 Core | ✅ Session 60 — gebaut; Schema v9 (`calendar_events.description` ergänzt, Sync liest sie mit) |
| Teilen: Disponere als Empfangsziel (`ACTION_SEND`, natives MethodChannel) | 🟢 Enhancement | ✅ Sessions 43–47 |
| Teilen: aus Disponere heraus (`share_plus`) | 🟢 Enhancement | ✅ Sessions 43–47 |
