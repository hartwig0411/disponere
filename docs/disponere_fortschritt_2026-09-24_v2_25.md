...
## Session 55 - 21. August 2026 (Nachtrag: Bild in der Eintragskarte klein - Ideen-Eingang E-01; reine Doku-Session)

### Charakter
- **Reine Doku-/Triage-Session im Chat-Modus, kein Code.** Ein Ideen-Eingang (E-01),
  aus Disponere selbst gemeldet, wurde triagiert und ins Anforderungsdokument ueberfuehrt.
  Ein docs-Commit (Anforderungen v6.7 + Ideen-Eingang), danach dieser Fortschrittsblock.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`1b7acc4`** ueber frischen `--depth 1`-Clone
  verifiziert (Fortschritt Session 54 + Anforderungen v6.6, zitiert `3e2b343`; Kette sauber).
- **Nachlauf:** Die beiden Dokumente der Vor-Session (v6.7 + Ideen-Eingang) waren wegen eines
  Download-/Ablage-Problems nie auf Vega gelandet und damit nie in git. In dieser Session
  sauber benannt bereitgestellt, abgelegt und committet. Alte v6.6 bleibt liegen (Repo behaelt
  alle Versionen); im Claude-Projekt zog eine verwaiste `claude_`-Dublette der v6.7 mit, die
  beim Neuupload entfernt wird.

### Entscheidung (mit Steffen bestaetigt, aus der Vor-Session uebernommen)
- **E-01 -> VERSIO. Bild-Attachment in der Eintragskarte als kleines Vorschau-Quadrat.**
  Kompakte quadratische Miniatur, die das Motiv zeigt (kein generisches Symbol-Icon, damit
  auf einen Blick erkennbar ist, *welches* Bild). Antippen oeffnet weiterhin das Vollbild
  (bestehender Viewer). Gilt in Journal- und Tag-Ansicht gleich. Reine Anzeige-Aenderung an
  der schon gebauten Bild-Ablage - kein Schema-Eingriff.

### Anforderungsdokument
- **v6.6 -> v6.7.** Neuer Aenderungsblock "gegenueber Version 6.6" (Bild-Anzeige verkleinert);
  Feature 1 "Teilen & Bilder" um die Vorschau-Quadrat-Zeile ergaenzt. Kumulativ, v6.6-Inhalt
  erhalten.

### Ideen-Eingang
- **E-01 aus den offenen Eingaengen entfernt**, als erste Zeile ins neue Kurzprotokoll
  "Einsortiert" ueberfuehrt (-> VERSIO, 21.08.2026, verweist auf v6.7). Offene Eingaenge
  jetzt leer. (Kopf-Stand noch 13.08. - inhaltlich auf 21.08.; Datum beim naechsten Anfassen
  nachziehen.)

### Offen / als Naechstes
- **Umsetzung E-01** (Vorschau-Quadrat in der Eintragskarte) offen fuer eine kuenftige
  Coding-Session - reine Anzeige, kleiner Umfang (`entry_card` o. ae.).
- **`docs:`-Durchgang:** N3/F2/F3 (Status-Drift, Design Paragraph 4a, `app_colors`-Kommentar).
- **Nach Ermessen:** N2 (`huawei_ml_text` raus), N4, N5, Trivia, F4/F5/F6.
- **Fable-5-Zweitmeinung:** self-contained Briefing, bewusst zuletzt vor v1.0.

### Commits (heute)
- `8bed7de` - docs: requirements v6.7 + ideas intake E-01 - small image preview in entry card
- `docs:` - Fortschritt Session 55 v2_13 (dieser Commit; zitiert `8bed7de`)

---

## Session 56 — 23. August 2026 (Coding-Session: E-01 — Bild als kompaktes Vorschau-Quadrat)

### Charakter
- **Coding-Session im Chat-Modus (Cowork).** Umsetzung des in Session 55 triagierten
  Ideen-Eingangs E-01 (Anforderungen v6.7). Komplette Dateien; feat- dann docs-Commit.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`9a27435`** über frischen `--depth 1`-Clone verifiziert
  (Fortschritt Session 55 v2_13, zitiert `8bed7de`; Kette sauber).
- Ist-Zustand gelesen: `entry_card.dart` (Journal-Miniatur: volle Breite, `maxHeight 220`,
  `BoxFit.cover`, eigener Tap → `ImageViewerScreen`), `tag_view_screen.dart` (`_EntryImage`:
  `maxHeight 160`, **nicht** antippbar), `image_viewer_screen.dart`, `attachment.dart`,
  `app_colors.dart`. **Befund:** zwei divergierende Bild-Darstellungen; die Tag-Ansicht-Miniatur
  öffnete bisher kein Vollbild. Reine Anzeige-Sache, kein Schema-Eingriff.

### Erledigt (Umsetzung)
- **Neu `lib/widgets/entry_image_thumb.dart`** — `EntryImageThumb`, die *eine* geteilte
  Vorschau-Quadrat-Miniatur: **72 px** Kantenlänge, Motiv quadratisch beschnitten
  (`BoxFit.cover`, zentriert), feiner Rahmen (`AppColors.border`), `borderRadius 10`,
  `cacheWidth 216` (~3× Kante, scharf bei hoher Pixeldichte, sparsam). Eigener
  `GestureDetector`-Tap → unveränderter `ImageViewerScreen`. Fehlerfall: `broken_image`-Icon
  (statt Text, der im 72-px-Quadrat nicht passt).
- **`entry_card.dart`** — lokale `_EntryThumbnail`-Klasse entfernt, auf `EntryImageThumb`
  umgestellt; tote Importe (`dart:io`, `image_viewer_screen`) raus, `entry_image_thumb` rein.
- **`tag_view_screen.dart`** — lokale `_EntryImage`-Klasse entfernt, auf `EntryImageThumb`
  umgestellt; `dart:io`-Import raus, `entry_image_thumb` rein. Die Tag-Ansicht-Miniatur ist
  damit **erstmals antippbar** (Vollbild), wie in v6.7 gefordert — Ansehen, kein Bearbeiten,
  konsistent zu den dort bereits antippbaren Tag-Chips.

### Auf dem MatePad bestätigt
- Kompaktes Quadrat in Journal **und** Tag-Ansicht, Motiv erkennbar, Antippen → Vollbild in
  beiden (Steffen: „ist getestet").

### Reversible Ermessensentscheidungen (markiert)
- **Ein geteiltes Widget** statt zwei divergierender Kopien — eine Quelle der Wahrheit für die
  Bild-Darstellung, damit ein Bild überall gleich aussieht (genau die Drift, die E-01 schließt;
  dieselbe Klasse Aufräumen wie `remapTagList` in Session 54). Fasst deshalb neben `entry_card`
  auch `tag_view_screen` an (statt „nur `entry_card`"). Umkehrbar.
- **Kantenlänge 72 px, `BoxFit.cover` zentriert** — die in v6.7 markierten reversiblen Details.
- **`cacheWidth 216`** (~3× Kante) statt der alten 1080/720 — die Miniatur braucht keine volle
  Auflösung.
- **Fehler-Icon statt Text** im Quadrat.

### Angefasste Dateien
- Neu: `lib/widgets/entry_image_thumb.dart`.
- Geändert: `lib/screens/journal/widgets/entry_card.dart` (Miniatur → geteiltes Widget, tote
  Importe entfernt).
- Geändert: `lib/screens/tags/tag_view_screen.dart` (Miniatur → geteiltes Widget, jetzt
  antippbar, `dart:io`-Import entfernt).

### Werkzeug
- Kein Flutter-Toolchain in der Cowork-Umgebung — Klammer-Prüfer (Mode-Stack: Zeilen-/
  verschachtelte Blockkommentare, raw/Triple-Strings, `${...}`-Interpolation mit vollem
  Klammer-Stack) gegen 9 Positiv-/Negativfälle validiert (Selbsttest bestanden); alle drei
  Dateien balanciert.

### Anforderungsdokument
- **v6.7 → v6.8.** Neuer Änderungsblock „gegenüber Version 6.7" (E-01 umgesetzt); die
  Feature-Zeile „Bild-Anzeige als kleines Vorschau-Quadrat" von ⏳ auf **✅ Session 56** gesetzt,
  die Feature-Beschreibung um „umgesetzt Session 56" ergänzt. Kumulativ, v6.7-Inhalt erhalten.

### Offen / als Nächstes
- **`docs:`-Durchgang:** N3/F2/F3 (Status-Drift „Bild als Eintragsinhalt" + zwei „Teilen"-Zeilen
  von ⏳ auf ✅, Design Paragraph 4a, `app_colors`-Kommentar).
- **Nach Ermessen:** N2 (`huawei_ml_text` raus), N4, N5, Trivia, F4/F5/F6.
- **Fable-5-Zweitmeinung:** self-contained Briefing, bewusst zuletzt vor v1.0.

### Commits (heute)
- `feat:` - E-01: small square image preview via shared thumbnail widget (Session 56)
- `docs:` - Fortschritt Session 56 v2_14 + Anforderungen v6.8 (dieser Commit; zitiert den feat-Commit)

---

## Session 57 - 24. August 2026 (Doku-Durchgang N3/F2/F3 - Status-Drift bereinigt; reine Doku-Session)

### Charakter
- **Reine Doku-Session im Chat-Modus, kein Code, kein Verhalten.** Drei Nachtraege aus der
  Fable-Review vom 12.08. (N3/F2/F3) abgearbeitet: Status-Marker nachgezogen, Design-Doc
  praezisiert, ein Quelltext-Kommentar korrigiert. Ein Inhalts-docs-Commit, danach dieser
  Fortschrittsblock.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`9a099c9`** ueber frischen `--depth 1`-Clone verifiziert
  (Fortschritt Session 56 v2_14 + Anforderungen v6.8, zitiert den E-01-feat `e80f48e`; Kette
  sauber).
- Ist-Stand der drei Ziele vor dem Schreiben geprueft, damit nichts "repariert" wird, das
  laengst gebaut ist.

### Erledigt (Umsetzung)
- **N3 - Status-Drift in den Anforderungen.** Vier in den Sessions 43-47 gebaute Zeilen standen
  noch auf "geplant": Block-Kopf "Teilen & Bilder", Tabellenzeile "Bild als Eintragsinhalt"
  (Schema v7), "Teilen: Empfangsziel" und "Teilen: aus Disponere heraus". Alle auf "gebaut"
  gehoben; die zwei veralteten Selbst-Notizen zur Drift entfernt. Der echt-geplante Rest
  (Perlenkette v2.0, Dokument-Import, Backup, Mindmap) bleibt bewusst offen.
- **F2 - Design-Doc `disponere_design_v2_0.md` (in place).** Abschnitt 4a an das gebaute
  Verhalten angepasst: offene, heute faellige Aufgaben stehen als einzige zugelassene Ergaenzung
  auch im heutigen Journal-Block (Termine und ueberfaellige Aufgaben nur im Heute-Panel,
  erledigte aus beidem raus; Anforderungen v6.1 Abschnitt 2). Status-Kopf von "noch nicht
  umgesetzt" auf "umgesetzt" nachgezogen. Dunkel-Chrome-Ausnahme (Foto-Oberflaechen:
  Vollbild-Viewer, Scrim-Hintergrund im Eintrags-Sheet) als einzige bewusste Abweichung vom
  Hell-Prinzip Abschnitt 1 festgehalten.
- **F3 - `app_colors.dart`-Kommentar** von "Design v1.0" / `design_v1_0.md` auf v2.0 korrigiert.
  Reine Kommentarzeile; das "v1.0" fuer den nicht geplanten Dunkelmodus (App-Version) bleibt
  bewusst stehen.

### Anforderungsdokument
- **v6.8 -> v6.9.** Neuer Aenderungsblock "gegenueber Version 6.8" (Doku-Durchgang N3/F2/F3);
  vier Statuszeilen auf gebaut, zwei Drift-Notizen entfernt. Kumulativ, v6.8-Inhalt erhalten.

### Werkzeug
- Klammer-Pruefer (Mode-Stack) neu aufgesetzt - diesmal mit echtem Kontext-Stack, nachdem die
  flache Variante bei `${...}`-Interpolation den String-Kontext verlor (nach dem schliessenden
  `}` als Code statt als String weitergelesen). Gegen 3 Positiv- und 3 Negativfaelle validiert
  (Selbsttest bestanden). `app_colors.dart` balanciert; `cmp`/`diff` zum Repo zeigten exakt die
  zwei Kommentarzeilen. Nach dem Push HEAD `6cad0aa` ueber frischen Clone bestaetigt, alle drei
  Dateien byte-identisch.

### Offen / als Naechstes
- **Nach Ermessen:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only), N5
  (Kaltstart-Race), Trivia T1-T3, F4/F5/F6.
- **Fable-5-Zweitmeinung:** self-contained Briefing, bewusst zuletzt vor v1.0.
- **Aus dem Realbetrieb:** was Disponere im taeglichen Gebrauch noch meldet (Ideen-Eingang).

### Commits (heute)
- `6cad0aa` - docs: N3/F2/F3 status catch-up - requirements v6.9, design v2.0 4a+status+dark-chrome, app_colors comment
- `docs:` - Fortschritt Session 57 v2_15 (dieser Commit; zitiert `6cad0aa`)

---

## Session 58 - 26. August 2026 (Doku-Reconcile: CoWork-Divergenz aufgeloest, v6.10 = N3 + E-02; reine Doku-Session)

### Charakter
- **Reine Doku-Session im Chat-Modus, kein Code, kein Verhalten.** Aufgeloest wurde eine
  Divergenz aus einer CoWork-Session vom 26.08.: dort war parallel eine **zweite, konkurrierende
  v6.9** entstanden (E-02, Wochenauswertung "Nicht Auswerten"), abgezweigt von v6.8 und blind fuer
  Session 57s v6.9 (N3). Zwei gleichnamige v6.9 mit nicht-ueberlappendem Inhalt. Zusammengefuehrt
  zu **v6.10 = Repo-v6.9 (N3/F2/F3) + E-02**. Ein Inhalts-docs-Commit, danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`1e1459a`** ueber frischen `--depth 1`-Clone verifiziert
  (Fortschritt Session 57 v2_15, zitiert `6cad0aa`; Kette sauber).

### Ausgangslage (das Problem)
- Die CoWork-Session hatte E-02 korrekt triagiert, aber nur als **Projekt-Dateien** geschrieben
  (`claude_`-Praefix) - **nichts committet**. E-02 lebte damit ausschliesslich im Projekt, nicht
  im Repo (HEAD stand auf `1e1459a`, nichts von CoWork dahinter).
- Ein frueherer Rat aus diesem Chat - die `claude_`-praefixierte Anforderungsdatei als blosses
  Download-Artefakt zu loeschen - war **falsch** und wurde korrigiert: die Datei war der
  **einzige Traeger** von E-02. Beide gestrandeten CoWork-Dateien (Anforderungen + Ideen-Eingang)
  ueber Chat-Karten gesichert, weil der Viewer-Download bei CoWork-Projektdateien klemmt.

### Erledigt (Umsetzung)
- **Drei-Wege-Analyse** gegen den gemeinsamen Vorfahren **v6.8**: N3-Diff (v6.8->Repo-v6.9) und
  E-02-Diff (v6.8->CoWork-v6.9) isoliert und auf Ueberlappung geprueft. N3 und E-02 fassen
  **verschiedene Anker** an (N3 in Header/Feature-Status; E-02 in Kernkonzept 8, neuer Changelog,
  neue Feature-Zeile) - konfliktfrei.
- **v6.10 gebaut.** Basis = Repo-v6.9 (unveraendert committet). Die drei E-02-Fragmente
  (Changelog-Block, Kernkonzept-8-Bullet, Feature-Tabellenzeile) **byte-exakt aus der CoWork-Datei
  extrahiert** (kein Abtippen deutscher Sonderzeichen) und **content-verankert mit Guards**
  eingespliced. Die belanglose Klammer-Umstellung aus CoWork bewusst weggelassen. Kontroll-Diff
  v6.9->v6.10: ausschliesslich Header + die drei E-02-Einfuegungen, N3 vollstaendig erhalten.
- **Ideen-Eingang reconciled.** E-02 unter "Einsortiert" (rein additiv gegenueber der Repo-Version),
  Referenz auf **v6.10** und "gegenueber v6.9" korrigiert.

### Entscheidungen
- **Versionskollision aufgeloest:** Repo-v6.9 (N3) bleibt wie committet stehen; E-02 wird **v6.10
  obenauf**. Damit lineare Historie, N3 und E-02 gemeinsam in einer Datei.
- **E-02 inhaltlich unveraendert** (aus der CoWork-Triage uebernommen): Filter auf **Tag-Ebene**
  (Eintrag bleibt, solange er mind. einen nicht-ausgeschlossenen Tag traegt), Ausschlussliste als
  **globale, persistente Einstellung** (pro Woche vorbefuellt, beim Aendern zurueckgeschrieben).
  Beide Gabelungen damit geklaert.

### Werkzeug
- Merge per Python mit **Anker-Splice-Guards**: Anker-Eindeutigkeit vor/nach, Gegenproben auf
  N3-Erhalt (Empfangsziel-Zeile, N3-Status-Marker, Dunkel-Chrome-Ausnahme) und korrekte
  Changelog-Reihenfolge (6.9 vor 6.8 vor 6.7). Fragmente byte-exakt extrahiert statt getippt.

### Offen / als Naechstes
- **E-02-Umsetzung** als eigene halbe **Coding-Session**: UI-Feld "Nicht Auswerten" + Filter auf
  Tag-Ebene beim Kontext-Aufbau + globale Ausschlussliste (Ablage reversibel, kein Schema, kein
  Prompt-Umbau). Triage und Coding bleiben getrennt.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only), N5 (Kaltstart-Race),
  Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst zuletzt vor v1.0.
- **Projekt-Sync:** v6.10 rein, v6.8/v6.9 und das `claude_`-Artefakt raus, Ideen-Eingang aktualisieren.

### Commits (heute)
- `984d6d3` - docs: requirements v6.10 - E-02 weekly-eval tag exclusion (Nicht Auswerten field), consolidated from cowork triage; ideas intake E-02 settled
- `docs:` - Fortschritt Session 58 v2_16 (dieser Commit; zitiert `984d6d3`)

---

## Session 59 - 28. August 2026 (Architektur: drittes Bruecken-Bein Termin->Eintrag geklaert; reine Architektur-Session)

### Charakter
- **Reine Architektur-Session im Chat-Modus, kein Code, kein Verhalten.** Das dritte, fehlende
  Bein der Perlenketten-Bruecke geklaert: **Termin -> Eintrag**. Alle Gabelungen aufgeloest,
  Umsetzung folgt in eigener Coding-Session (Split bleibt). Ein Inhalts-docs-Commit
  (Anforderungen v6.11), danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`fce843a`** ueber frischen `--depth 1`-Clone verifiziert
  (Fortschritt Session 58 v2_16, zitiert `984d6d3`; Kette sauber).

### Ausgangslage (die Luecke)
- Frage aus dem Tagesgeschaeft: Kann man aus einem Termin einfach einen Journaleintrag machen?
  Befund: **Nein.** Die Bruecke verband bisher nur **Eintrag <-> Aufgabe** (v6.5, Session 53).
  Der Termin ist das dritte Objekt der Perlenkette (Kernkonzept 1), stand aber aussen vor -
  reine read-only-Anzeige aus dem Kalender, kein Weg zu einer eigenen Notiz.
- Steffen: im Tagesgeschaeft ein **echter Showstopper** -> direkt zur Umsetzung entschieden,
  kein Umweg ueber den Ideen-Eingang (kein E-03). Rueckt damit **vor E-02** in die Warteschlange.

### Erledigt (Architektur geklaert)
- **Ausloeser = Long-Press auf die Terminkarte** -> neue, einzige Aktion "Zu Eintrag machen".
  Der Termin bekommt dafuer **erstmals ein Aktionsmenue** (bisher reine Anzeige) - der einzige
  neue UI-Baustein.
- **Sheet = bestehendes `bridge_sheet.dart`**, vorbefuellt und kuerzbar. Inhalt = **Titel**,
  falls vorhanden nach einer Leerzeile die **Beschreibung** (`calendar_events.description`).
  **Ort und Uhrzeit nicht in den Text** (Ort bewusst weggelassen).
- **Datierung = fix auf den Termintag** (`display_day` = Termintag, **kein Datumswaehler**).
  Der "naechster Tag"-Fall ist bewusst der andere Weg -> Aufgabe.
- **Verbindung = geteilter Tag:** Eintrag **erbt die `event_tags`** (Kopie der Liste). Termin
  bleibt unveraendert (read-only ohnehin zwingend).
- **Doppelung akzeptiert:** Termin und abgeleiteter Eintrag am selben Tag; in der Tag-Ansicht
  eine Perle. **Kein Ausblenden** des Termins - das erforderte eine an die Event-ID gekoppelte,
  persistente Ausblend-Liste gegen den read-only-Sync und liefe dem Bruecken-Prinzip zuwider;
  bewusst nicht fuer v1.0.

### Entscheidungen (Gabelungen geschlossen)
- **Doppelung:** beide stehen lassen (Termin und Eintrag am Termintag) - konsistent zur Bruecke,
  keine neue Komplexitaet.
- **Datum:** fix auf Termintag, kein Picker.
- **Textinhalt:** Titel + Beschreibung (falls vorhanden), kuerzbar im Sheet; Ort raus.
- **Schema:** keiner. Nur vorhandene Felder (`calendar_events.description`,
  `entries.display_day`, `event_tags` / `entry_tags`). Kein v8->v9.

### Datenlage bestaetigt
- Kalender-Architektur v1.0 geprueft: `calendar_events` haelt `title`, `description?`,
  `location?` bereits lokal. Den Textinhalt mitnehmen ist damit **kostenlos** - kein Sync-Umbau,
  kein Schema-Umbau.

### Anforderungsdokument
- **v6.10 -> v6.11.** Neuer Aenderungsblock "gegenueber Version 6.10" (drittes Bruecken-Bein,
  Architektur entschieden, noch nicht gebaut) plus eine Feature-Tabellenzeile (Status geplant).
  Basis byte-exakt aus v6.10; Anker-Splice mit Guards. Kontroll-Diff: ausschliesslich
  Versionszeile, Stand-Zeile, neuer Block, neue Tabellenzeile - v6.10-Inhalt (E-02, N3)
  unangetastet.

### Offen / als Naechstes
- **Coding-Session: drittes Bruecken-Bein** Termin->Eintrag bauen (Long-Press-Menue auf der
  Terminkarte + `bridge_sheet` mit Titel/Beschreibung + `display_day` = Termintag + `event_tags`
  vererben). Bewusst **vor E-02**.
- **Danach E-02** (Umsetzung "Nicht Auswerten").
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only), N5
  (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst zuletzt vor v1.0.

### Commits (heute)
- `c9e449c` - docs: requirements v6.11 - third bridge leg event->entry, architecture decided (not yet built)
- `docs:` - Fortschritt Session 59 v2_17 (dieser Commit; zitiert `c9e449c`)

---

## Session 60 - 29. August 2026 (Coding: drittes Bruecken-Bein Termin->Eintrag gebaut; Schema v9 calendar_events.description)

### Charakter
- **Coding-Session: das dritte Bruecken-Bein Termin -> Eintrag ist gebaut.** Umsetzung der in
  Session 59 geklaerten Architektur; der Split Architektur/Coding blieb sauber. Ein Feat-Commit
  (Code), danach dieser docs-Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`e8d12e8`** ueber frischen `--depth 1`-Clone verifiziert
  (Fortschritt Session 59 v2_17, zitiert `c9e449c`; Kette sauber).

### Fork vor dem Bauen (wichtig)
- Beim Lesen der Ist-Dateien fiel auf: die in Session 59 als **"Datenlage bestaetigt"** notierte
  Annahme stimmte **nicht**. Das echte Schema (v8) hat in `calendar_events` **keine**
  `description`-Spalte; der Google-Sync liest nur `summary` und `location`; im Code kommt
  `description` **kein einziges Mal** vor. Die Kalender-Architektur (v4) plante die Spalte zwar
  (`title`, `description?`, `location?`), der implementierte Code hatte sie aber nie -
  **Design-Doku und Code waren auseinandergelaufen**.
- Vor dem Bauen vorgelegt statt still entschieden. Zwei Wege: **A** Titel-only (null Umbau),
  **B** Beschreibung richtig nachruesten (Schema v8->v9). **Steffen: B.** Damit wird die schon in
  v4 geplante Beschreibung real - die Doku-Luecke ist mitgeschlossen.

### Erledigt (gebaut, 6 Dateien)
- **`calendar_event.dart`** - Modell bekommt `description`.
- **`google_calendar_service.dart`** - Sync liest `item['description']` mit (kein `fields`-Filter
  im Request, kommt gratis mit).
- **`journal_repository.dart`** - **Schema v8 -> v9**: `description`-Spalte in `calendar_events`,
  Migration `_addEventDescriptionColumn` (`ALTER TABLE ... ADD COLUMN`); Lesen im Hydrate,
  Schreiben im Sync-Upsert. `_onCreate` legt die Spalte fuer Neuinstallationen direkt mit an.
- **`bridge_sheet.dart`** - neuer Einstieg `showEventToEntrySheet` (Zwilling zu
  `showTaskToEntrySheet`): Inhalt = Titel, bei vorhandener Beschreibung nach Leerzeile
  drangehaengt; Tags geerbt; Datum **fix, schreibgeschuetzt** (neuer `dateEditable:false`-Pfad +
  `_ReadOnlyDateRow`).
- **`event_card.dart`** - optionales `onLongPress`, Karte in `InkWell` gewickelt.
- **`today_panel.dart` + `past_day.dart`** - reichen `onLongPressEvent` an die Karte durch.
- **`journal_screen.dart`** - `_showEventActions` (Long-Press -> Menue mit einziger Aktion
  "Zu Eintrag machen") + `_eventToEntry` -> `_addEntry(content, tags, displayDay:)`. An beiden
  Kartenorten (Heute-Panel + vergangene Tage) verdrahtet.

### Validierung
- Klammer-Checker (Mode-Stack, neu gebaut und gegen Positiv/Negativ-Faelle geprueft) auf allen
  acht Dateien gruen. `description`-Fluss end-to-end kontrolliert (Modell -> Sync -> Schema/
  Migration -> Persistenz -> Sheet).
- Nach dem Push: **frischer Clone**, HEAD `d5a4b72`, `cmp` gegen die gelieferte
  `journal_repository.dart` -> byte-identisch. Auf dem MatePad getestet (Steffen): funktioniert.
- Betriebshinweis: bestehende gespiegelte Termine haben `description = NULL`, bis einmal neu
  synchronisiert wurde; Titel-Uebernahme klappt sofort.

### Anforderungsdokument
- **v6.11 -> v6.12.** Neuer Block "gegenueber Version 6.11": drittes Bein **gebaut**, plus die
  **ehrliche Korrektur** (v6.11 sagte "kein v8->v9"; noetig war es doch, weil Doku und Code
  auseinandergelaufen waren). Feature-Tabellenzeile auf "gebaut (Session 60)"; beide Schema-Zeilen
  auf v9. Basis byte-exakt aus v6.11, Anker-Splice mit Guards.

### Ideen-Eingang
- **E-03 aufgenommen** ("Tags im Bruecken-Sheet editierbar", Status NEU, halbe Session): die
  geerbten Tags nicht nur read-only zeigen, sondern vor dem Anlegen aendern koennen. Triage-Notiz
  zur Rueckschreib-Frage vermerkt; wandert bei der naechsten Triage weiter.

### Offen / als Naechstes
- **E-02** (Umsetzung "Nicht Auswerten") - naechstes Warteschlangen-Element.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only), N5
  (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst zuletzt vor v1.0.
- **E-03** wartet auf Triage (nicht v1.0-blockend).
- Kosmetik: `event_card.dart` innerer Block durch die neue `InkWell`-Schicht 2 Spaces
  unter-eingerueckt - kompiliert sauber, `dart format` buegelt es beim ersten Lauf glatt.

### Commits (heute)
- `d5a4b72` - feat: event->entry bridge (third leg) + calendar_events.description (schema v9)
- `docs:` - Fortschritt Session 60 v2_18 (dieser Commit; zitiert `d5a4b72`)

---

## Session 61 - 04. September 2026 (Tagesgeschaeft: Termin-Leerzeilen + groesseres Bearbeiten-Sheet; E-03 gebaut; Punkt 4 Weg A entschieden)

### Charakter
- Coding-Session aus dem Tagesgeschaeft: drei konkrete Aergernisse vom MatePad
  behoben, eine Architektur-Gabelung (Tintenmodus) entschieden aber bewusst
  NICHT gebaut. Ein Feat-Commit (Code), danach dieser docs-Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD `9e4b89d` ueber frischen `--depth 1`-Clone
  verifiziert (docs Session 60 v2_18, zitiert `d5a4b72`; Kette sauber:
  `9e4b89d` -> `d5a4b72` -> `e8d12e8`).

### Ausgangslage (vier Punkte vom MatePad)
- Steffen meldete vier Punkte, 1/2/4 als "sehr dringend":
  1. Termin->Eintrag uebernimmt E-Mail-Beschreibungen mit vielen Leerzeilen ->
     Eintraege viel zu gross (Beispiel: weitergeleitete Teams-Einladung).
  2. Bearbeiten-Sheet zu klein (`maxLines: 4`) -> auf dem 12,2"-Tablet kaum nutzbar.
  3. `#Tag` bei der Uebernahme aendern koennen.
  4. Tintenmodus: keine zweite Seite / kein Autoscroll -> im Meeting stark
     eingeschraenkt.

### Klaerung vor dem Bauen
- **Punkt 3:** Der Bearbeiten-Sheet hat bereits ein editierbares Tag-Feld
  (`TagAutocompleteField`, vorbefuellt). Read-only sind Tags nur im
  **Bruecken-Sheet** bei der Uebernahme selbst - genau das ist **E-03**.
  Rueckfrage gestellt; Steffen will es auch dort -> E-03 gebaut.
- **Punkt 4:** echte Architektur-Gabelung, vor dem Bauen vorgelegt. **Weg A**
  (Endlos-Rolle mit Autoscroll) vs. **B** (feste Seiten). **Steffen: Weg A.**
  Bewusst NICHT in dieser Session gebaut - es ist die Kern-Schreibflaeche und
  verdient eine eigene fokussierte Session.

### Erledigt (gebaut, 3 Dateien)
- **Punkt 1 - `bridge_sheet.dart`:** neuer Verdichter `_compactDescription` fuer
  die Termin-Beschreibung. Zeilen rechts beschnitten, Folgen leerer Zeilen auf
  **eine** reduziert, Rand-Leerzeilen weg; Absatzgrenzen bleiben.
  `showEventToEntrySheet` nutzt ihn beim Seed. (Umkehrbar markiert: falls gar
  keine Leerzeilen gewuenscht, Schwelle 2->1, Einzeiler.)
- **Punkt 2 - `journal_screen.dart`:** Inhalt-Feld im Bearbeiten-Sheet von
  `maxLines: 4` auf `minLines: 8 / maxLines: 16`; der ganze Sheet-Koerper in
  `SingleChildScrollView` -> kein Ueberlauf mit Tastatur + langem Text.
- **E-03 (Punkt 3) - `bridge_sheet.dart` + `journal_screen.dart` +
  `task_overview_screen.dart`:** die geerbten Tags im Bruecken-Sheet sind jetzt
  editierbar (statt read-only Chips) - ueber dasselbe `TagAutocompleteField` wie
  im Bearbeiten-Sheet, vorbefuellt mit den geerbten Tags, **immer sichtbar**
  (auch bei tag-loser Quelle). Gilt fuer alle drei Bruecken-Richtungen
  (Eintrag->Aufgabe, Aufgabe->Eintrag, Termin->Eintrag). `_showBridgeSheet`
  bekommt `knownTags` und einen `tagController`; `onSubmit` reicht die editierten
  Tags heraus (Signatur `(DateTime? day, List<String> tags)`). Kanonisierung:
  die beiden ->Eintrag-Wege ueber `_addEntry` / `canonicalizeAll` wie gehabt,
  Eintrag->Aufgabe **neu** via `task.copyWith(tags: canonicalizeAll(...))` im
  Aufrufer. `_ReadOnlyTagChip` entfernt (nun ungenutzt).

### Validierung
- Klammer-Checker (Mode-Stack, neu gebaut, Positiv/Negativ-Selbsttest bestanden)
  auf allen drei Dateien gruen.
- Nach dem Push: frischer Clone, HEAD `d031935`, `cmp` gegen alle drei
  gelieferten Dateien -> byte-identisch. Auf dem MatePad getestet (Steffen):
  "schon sehr viel besser".
- Hinweis: `dart format` buegelt beim ersten Lauf eine Einrueckung im
  Bearbeiten-Sheet-Wrap (SingleChildScrollView) glatt - kompiliert sauber.

### Anforderungsdokument
- **v6.12 -> v6.13.** Neuer Block "gegenueber Version 6.12": zwei
  Tagesgeschaefts-Fixes (Leerzeilen-Verdichtung, groesseres Bearbeiten-Sheet) +
  E-03 gebaut + Punkt 4/Weg A als Architektur-Entscheidung (noch nicht gebaut).
  Basis byte-exakt aus v6.12, Anker-Splice; Versionszeile und Stand-Zeile
  aktualisiert.

### Ideen-Eingang
- **E-03 von NEU auf gebaut (Session 61).** Achtung: die im Projekt liegende
  Kopie des Ideen-Eingangs enthaelt E-03 noch nicht (aelter als die Aufnahme in
  Session 60) - **lokal in `disponere_ideen_eingang_v1_0.md` nachziehen** und
  auf "gebaut" setzen.

### Offen / als Naechstes
- **Punkt 4 / Weg A bauen** (Tintenmodus Endlos-Rolle mit Autoscroll) - eigene
  fokussierte Coding-Session. Architektur steht: Striche in Inhalts-Koordinaten
  (Viewport + Scroll-Offset); Autoscroll nahe unterem Rand; `InkData.height` =
  volle Rollenlaenge; Auswertung sehr hoher Rollen ueber ueberlappungsfreie
  Kacheln; `_fitLoadedInkToCanvas` fittet kuenftig nur auf die Breite (Hoehe =
  gespeicherte Laenge). Betroffene Dateien voraussichtlich: `drawing_screen.dart`,
  `ink_data.dart`, `ink_painter.dart`, `ink_renderer.dart`.
- **E-02** (Umsetzung "Nicht Auswerten") - weiter in der Warteschlange.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
  zuletzt vor v1.0.

### Commits (heute)
- `d031935` - feat: collapse blank lines on event->entry, larger edit sheet, editable tags in bridge (E-03)
- `docs:` - Fortschritt Session 61 v2_19 (dieser Commit; zitiert `d031935`)

---

## Session 62 - 08. September 2026 (Coding: eigene Notiz vs. geteilter Text getrennt - import_body, Schema v10)

### Charakter
- **Coding-Session im Chat-Modus.** Aus dem Tagesgeschaeft: In einem Eintrag,
  der geteilten/importierten Fremdtext und die eigene Notiz mischt (z. B. eine
  weitergeleitete Q4-Mail plus die eigene Loesung darueber), war die eigene
  Notiz optisch nicht vom Fremdtext zu unterscheiden - sie ging in der Textwand
  unter. Komplette Dateien; feat- dann docs-Commit.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. HEAD **`68e4464`** (docs Session 61 v2_19,
  zitiert feat `d031935`) ueber frischen `--depth 1`-Clone verifiziert; Kette
  sauber.
- Betroffene Dateien vor dem Schreiben gelesen: `journal_entry.dart`,
  `app_colors.dart`, `search_hit.dart`, `journal_repository.dart` (Schema,
  loadAll, upsert, Suche), `entry_card.dart`, `tag_view_screen.dart`,
  `journal_screen.dart` (Sheet/Share/Save), `share_receiver.dart`,
  `bridge_sheet.dart`.

### Gabelung vor dem Coden geklaert (mit Steffen)
- **Modell A: ein Eintrag, zwei Felder** - eigene Notiz oben, geteilter Text
  darunter (nicht B = zwei verknuepfte Eintraege). "Erkennbar zusammenhaengend"
  ist damit per Definition erfuellt: beide Teile sind ein Eintrag. Keine neue
  Eintrag-zu-Eintrag-Kante noetig.
- **Befund im Code, der die Gabelung ausloeste:** ein Eintrag hatte genau ein
  Textfeld (`content`), keinen Quellen-Marker, kein zweites Feld. Die App konnte
  gar nicht wissen, welcher Teil eigene Notiz und welcher Fremdtext war. Die
  Aufteilung ist damit strukturell (welches Feld), nicht inhaltlich - die App
  erkennt Import nicht am Aussehen des Textes.

### Erledigt (Umsetzung)
- **Schema-Migration v9 -> v10:** neue Spalte `entries.import_body` (nullable);
  `_onCreate` und `_onUpgrade` (`if (oldVersion < 10)`, Helfer
  `_addImportBodyColumn`) teilen dasselbe Schema. `content` behaelt seine
  Bedeutung (Steffens eigener Text) - **alle Altbestaende bleiben korrekt** und
  faerben sich nur in die Stimme um; kein Datenumbau.
- **Modell** (`journal_entry.dart`): Feld `importBody` neben `content` (bewusst
  getrennt, dieselbe Grenze wie `inkText`), `hasImportBody`, `copyWith`.
- **Repository:** `import_body` in `loadAll` und `_upsertInTxn` (faehrt beim
  `ConflictAlgorithm.replace` mit, sonst wuerde Bearbeiten den Import loeschen);
  Volltextsuche indiziert jetzt auch `import_body` (`SearchHitSource.importBody`),
  Reihenfolge content -> import -> inkText.
- **Darstellung** (`entry_card.dart` + `tag_view_screen.dart`, beide Render-
  Stellen): eigene Notiz (`content`) als Stimme in Blau-Schwarz (`AppColors.ink`);
  bei beiliegendem Import zusaetzlich leicht eingerueckt mit feiner Fuehrungslinie
  (`AppColors.guide`). Geteilter Text (`import_body`) darunter in zurueckhaltendem
  Grau (neuer Token `AppColors.importBody`), kleiner, nichts gekuerzt/eingeklappt.
- **Share-Intake** (`journal_screen.dart`): geteilter Text (ACTION_SEND) landet
  jetzt im getrennten Import-Feld; das Notizfeld bleibt leer und fokussiert. Das
  Eintrags-Sheet zeigt ein graues "GETEILTER TEXT"-Feld nur, wenn Import
  vorliegt. Leeren des Feldes loescht den Import bewusst nicht (Sicherheit wie
  bei der Tinten-Auswertung; Entfernen ueber Loeschen des Eintrags).
- **Termin -> Eintrag-Bruecke** (`bridge_sheet.dart` + Verdrahtung im Journal):
  der Termintext (Titel + Beschreibung) fuellt jetzt ebenfalls das Import-Feld
  (graues "TERMIN"-Feld im Sheet), das Notizfeld bleibt oben leer fuer eigene
  ToDos. `_showBridgeSheet` um ein optionales Import-Feld erweitert; `onCreate`
  des Termin-Beins traegt `importBody`. Die anderen zwei Bruecken-Beine
  unveraendert (Aufgabentitel ist eigener Text, kein Import).

### Auf dem MatePad bestaetigt
- Teilen-Weg und Termin-Weg zeigen die Aufteilung (eigene Notiz blau-schwarz
  ueber grauem Fremdtext). Steffen: passt. (Erwartungs-Klarstellung: die
  Aufteilung greift nur bei NEU geteiltem/gebrucktem Material, weil strukturell -
  ein alter, gemischt-in-content angelegter Eintrag bleibt eine Textwand.)

### Reversible Ermessensentscheidungen (markiert)
- **Einrueckung + Fuehrungslinie nur im gemischten Fall** (Import vorhanden); ein
  reiner Notiz-Eintrag steht flach in Blau-Schwarz - keine Linie an jedem
  Eintrag.
- **Stimme = `AppColors.ink` (`#2A2B48`)** - bewusst der schon fuer Tinte
  reservierte Token, semantisch "meine Stimme". Caveat-Handschriftschrift bewusst
  NICHT als Fliesstext (Lesbarkeit), hoechstens spaeterer Akzent. Steffens Wunsch
  "etwas mehr Blau" als kleiner Feinschliff notiert.
- **Import leeren loescht nicht** (copyWith-null = behalten) - Schutz vor
  versehentlichem Verlust des Rohmaterials.
- **Blau-Schwarz gilt rueckwirkend fuer alle getippten Eintraege** (bewusst;
  genau der Wunsch "alles Handgetippte abgesetzt").

### Angefasste Dateien
- Geaendert: `lib/models/journal_entry.dart`, `lib/models/search_hit.dart`,
  `lib/theme/app_colors.dart`, `lib/data/journal_repository.dart`,
  `lib/screens/journal/widgets/entry_card.dart`,
  `lib/screens/tags/tag_view_screen.dart`,
  `lib/screens/journal/journal_screen.dart`, `lib/widgets/bridge_sheet.dart`.

### Werkzeug
- Klammer-Pruefer neu aufgebaut (Mode-Stack: Zeilen-/verschachtelte
  Blockkommentare, raw/triple-Strings, `${...}`-Interpolation inkl. Rueckkehr in
  den String) und mit Positiv-/Negativ-Selbsttest validiert; alle acht Dateien
  balanciert.

### Offen / als Naechstes
- **E-04 (neu, Ideen-Eingang):** OneCalendar erscheint nicht im Teilen-Menue -
  Disponeres Manifest meldet nur `text/plain` + `image/*`; ein Kalendertermin
  wird als `text/calendar`/`.ics` geteilt, dafuer ist Disponere nicht
  registriert. Nachzug: Intent-Filter um den Kalender-Typ erweitern und `.ics`
  beim Empfang ins `import_body`-Feld leiten.
- **Mehr Blau in der Stimme** (Farbwert) - kleiner optionaler Feinschliff.
- **Punkt 4 / Weg A** (Tintenmodus Endlos-Rolle mit Autoscroll) - eigene
  fokussierte Coding-Session.
- **E-02** ("Nicht Auswerten") - weiter in der Warteschlange.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
  zuletzt vor v1.0.

### Anforderungsdokument
- **v6.13 -> v6.14.** Neuer Block "gegenueber Version 6.13": eigene Notiz vs.
  geteilter/importierter Text getrennt (Schema v10, `import_body`),
  Stimme-Styling in Journal/Tag-Ansicht, Termin->Eintrag-Bruecke fuellt das
  Import-Feld; E-04 (OneCalendar-Share) als neuer offener Punkt.

### Commits (heute)
- `e756cea` - feat: import_body (schema v10) - trennt eigene Notiz von geteiltem Text; Stimme-Styling in Journal/Tag-Ansicht; Termin->Eintrag-Bruecke fuellt import_body
- `docs:` - Fortschritt Session 62 v2_20 + Anforderungen v6.14 (dieser Commit; zitiert `e756cea`)

---

## Session 63 - 08. September 2026 (Coding, Teil II: Tintenmodus als scrollbare Endlos-Rolle mit Autoscroll - Weg A)

### Charakter
- **Coding-Session im Chat-Modus, Teil II des Tages.** Weg A (in Session 61
  entschieden) gebaut: die Kern-Schreibflaeche wird aus einer festen,
  bildschirmgrossen Einzelseite eine mitwachsende, vertikal scrollbare
  Endlos-Rolle mit Autoscroll. Ein Feat-Commit (eine Datei), danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. Part I des Tages (Session 62, import_body)
  vollstaendig nachgelesen; HEAD **`ee234ec`** (docs Session 62 v2_20, zitiert
  feat `e756cea`) ueber frischen `--depth 1`-Clone verifiziert; Kette sauber.
- Vor dem Bauen die echten Ist-Dateien gelesen: `ink_data.dart`,
  `ink_painter.dart`, `ink_renderer.dart`, `drawing_screen.dart`.

### Befund vor dem Bauen (enger als er klingt)
- `InkData` speichert Striche als **absolute Punkte** plus beliebige
  `width`/`height` - eine hohe Rolle passt **ohne Schema-Aenderung**.
- `InkLivePainter` malt in absoluten Koordinaten - fuer die Rolle unveraendert
  brauchbar; `ink_renderer`/`InkPreviewPainter` schneiden ohnehin auf die
  Bounding-Box zu.
- Ergebnis: Der Roll-Umbau betrifft im Kern **genau eine Datei**
  (`drawing_screen.dart`). Modell, Painter, Repository und der aufrufende
  Journal-Screen bleiben unangetastet; nur die Bedeutung von `height` wird
  "volle Rollenlaenge" statt "Bildschirmhoehe" - ueber den ganzen Code sicher.

### Gabelungen vor dem Coden (mit Steffen)
- **Kachelung fuer die Claude-Auswertung: spaeter.** Weg A nennt sie mit, aber
  sie betrifft nur die Auswertung sehr hoher Rollen, nicht die Schreibflaeche.
  Ohne sie laeuft die Auswertung weiter (ein Bild, auf 1568 px lange Kante
  skaliert); erst bei Extremlaengen sinkt die Aufloesung. Bewusst eigener
  Nachzug - am sinnvollsten, wenn real eine lange Rolle ausgewertet wurde und
  sichtbar ist, ab welcher Laenge es praktisch kippt.
- **Autoscroll aktiv folgend** gebaut (= die Entscheidung); sicherer Rueckfall
  waere "nur mitwachsen, selbst scrollen" (zwei Stellschrauben, kein Umbau).

### Erledigt (eine Datei: drawing_screen.dart, 581 -> 707 Zeilen)
- **Scrollbare Rolle:** Canvas in `SingleChildScrollView`; der `Listener` sitzt
  **in** der Rolle -> `localPosition` ist direkt Inhalts-Koordinate, keine
  Offset-Rechnerei. `RepaintBoundary` entfernt (war nur fuer die Groesse da; die
  Auswertung rendert vektoriell) - keine riesige Rasterebene bei langen Rollen.
- **Stift zeichnet, Finger scrollt:** eigenes `_StylusExcludedScrollBehavior`
  (`dragDevices` ohne `stylus`). Der M-Pencil rollt die Flaeche nicht weg -
  dieselbe Trennung wie die bestehende Palm-Rejection.
- **Autoscroll + Wachsen:** `_followPen` scrollt (jumpTo, 1:1) mit, sobald die
  Schreibzeile tiefer als das untere Band (`_followBandFactor` 0.28) kommt; die
  Rolle waechst nach unten mit einem halben Bildschirm Puffer
  (`_bottomPadFactor` 0.5), damit das Scroll-Ziel stets in der Strecke liegt.
- **Speichern:** `height` = volle Rollenlaenge (`_currentInk` liest die
  Inhaltsgroesse ueber den `_canvasKey` auf der Rollen-Box).
- **Laden geerbter Tinte:** `_fitLoadedInkToCanvas` -> `_fitLoadedInkToWidth`:
  fittet nur noch auf die **Breite**, oben beginnend; ein langer Eintrag oeffnet
  in voller Laenge zum Weiterschreiben statt gestaucht. Gleiche Breite ->
  Faktor 1, keine Verzerrung.

### Auf dem MatePad bestaetigt
- Steffen: "funktioniert". Rest im Praxisbetrieb beobachten. Reversible
  Stellschrauben (`_followBandFactor`, `_bottomPadFactor`) notiert, falls das
  Mitrollen zu eifrig oder traege wirkt.

### Korrektur einer eigenen Fehllesung (Ehrlichkeit)
- Beim Session-Start hatte ich "Anthropic-Organisation gesperrt" als
  **aktuellen** Blocker referenziert - tatsaechlich stammt das aus alten
  Session-Eintraegen (History). Die Doku haelt die Sperre bereits korrekt als
  **aufgehoben** fest (Guthaben in der Console bestaetigt), und der
  Auswerte-Pfad laeuft (Wochenauswertung im Einsatz). Es gab also **keinen
  stehenden Stale-Hinweis zu entfernen**; History-Eintraege bleiben unangetastet.

### Werkzeug
- Klammer-Pruefer (Mode-Stack) neu aufgebaut, Positiv-/Negativ-Selbsttest gruen,
  Datei balanciert.

### Offen / als Naechstes
- **Kachelung der Auswertung fuer sehr hohe Rollen** - eigener Nachzug, sobald
  eine lange Rolle real ausgewertet wurde.
- **E-04** (OneCalendar erscheint nicht im Teilen-Menue; `text/calendar`/`.ics`).
- **Mehr Blau in der Stimme** (Farbwert) - kleiner optionaler Feinschliff.
- **E-02** ("Nicht Auswerten") - weiter in der Warteschlange.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
...
### Anforderungsdokument
- **v6.14 -> v6.15.** Neuer Block "gegenueber Version 6.14": Tintenmodus als
  scrollbare Endlos-Rolle mit Autoscroll (Weg A) gebaut; Kachelung der
  Auswertung bewusst spaeter.

### Commits (heute, Teil II)
- `095ebb7` - feat: Tintenmodus als scrollbare Endlos-Rolle mit Autoscroll (Weg A) - Striche in Inhalts-Koordinaten, Stift zeichnet/Finger scrollt, Laden fittet auf Breite
- `docs:` - Fortschritt Session 63 v2_21 + Anforderungen v6.15 (dieser Commit; zitiert `095ebb7`)

---

## Session 64 - 09. September 2026 (Coding, Chat: E-02 "Nicht Auswerten")

### Charakter
- **Coding-Session im Chat-Modus.** E-02 aus der Warteschlange gebaut: ein Feld
  "Nicht Auswerten" schliesst Tags global aus der Wochenauswertung aus. Ein
  Feat-Commit (drei Dateien, eine neu), danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. Vorstand ueber frischen Clone verifiziert. Dabei
  zwei Abweichungen gefunden: die im Projekt hochgeladenen Docs hingen zwei
  Sessions zurueck (v2_18 statt v2_21), und der vorige HEAD war ein Feat-Commit
  ohne folgenden docs-Commit (Tintenmodus Weg A, `095ebb7`) - inzwischen von
  Steffen mit v2_21/v6.15 (`00eb91f`) sauber abgeschlossen. Kette danach
  lueckenlos.
- Projekt-Wissensbasis verschlankt: nur noch die je aktuelle Version jedes
  Dokuments (6 Kern-Dateien); alle Altversionen liegen im Repo unter docs/.

### Architektur vor dem Coden (Gabelung mit Steffen)
- E-02 war im Ideen-Eingang (26.08.) und in v6.15 bereits entschieden: Standard
  alle Tags, Feld "Nicht Auswerten" nimmt Ausnahmen, Filter auf Tag-Ebene,
  globale persistente Liste, kein Schema-Zwang.
- **Offene Gabelung, im Code gefunden:** Die Spec spricht nur von "Eintrag" -
  aber `week_context.dart` gibt auch Aufgaben und Termine mit ihren Tags in den
  Kontext, und "Tags dieser Woche" zaehlt Eintraege + Aufgaben + Termine. Nur
  Eintraege zu filtern, liesse ausgeschlossene Tags ueber Aufgaben/Termine doch
  in die Auswertung lecken.
  - **Weg A (woertlich):** nur Eintraege filtern - minimal, aber inkonsistent.
  - **Weg B (konsistent):** dieselbe Tag-Regel auf Eintraege, Aufgaben, Termine;
    ausgeschlossene Tags zusaetzlich aus Suffixen und Tag-Uebersicht entfernen.
  - Steffen: **Weg B.**

### Erledigt (drei Dateien)
- **`week_context.dart`** (Filter): `WeekContext.build` bekommt einen optionalen
  Parameter `excludedTags` (rueckwaertskompatibel - einziger Aufrufer ist der
  Auswertungs-Screen). Zwei Helfer: `keepByTags` (ein Objekt faellt nur heraus,
  wenn *alle* seine Tags ausgeschlossen sind; tag-lose nie) und `visibleTags`
  (ausgeschlossene Tags werden aus jedem Zeilen-Suffix und aus "Tags dieser
  Woche" entfernt). Gilt fuer Eintraege, Aufgaben, Termine gleich. Ein voll
  ausgeschlossener Eintrag zaehlt auch nicht mehr als "uebersprungene
  Handschrift".
- **`review_settings.dart`** (neu): die globale Ausschlussliste in
  `shared_preferences` (Schluessel `week_review_excluded_tags`). Kein
  Schema-Eingriff; leere Liste entfernt den Schluessel. `shared_preferences` war
  bereits Dependency - kein pubspec-Umbau.
- **`week_review_screen.dart`** (UI): Feld "Nicht auswerten" oben im Screen, das
  `TagAutocompleteField` aus E-03 wiederverwendet (Autocomplete ueber bekannte
  Tags). Beim Oeffnen aus der Liste vorbefuellt; "Uebernehmen" parst,
  kanonisiert, speichert und baut den Kontext neu, sodass die Vorschau den Filter
  sofort zeigt. Bekannte Tags aus Aufgaben, Kalender-Zuordnungen und den
  Eintraegen des Fensters.

### UX-Entscheidung (reversibel, Steffen zugestimmt)
- Zurueckschreiben an ein explizites "Uebernehmen" gehaengt statt bei jedem
  Tastendruck - klarer Moment, an dem der Filter greift, keine Races mit
  "Auswerten".

### Auf dem MatePad bestaetigt
- Steffen: "Funktioniert". Byte-Vergleich der drei Dateien gegen die
  Auslieferung nach dem Push: identisch.

### Werkzeug
- Klammer-Pruefer (Mode-Stack) neu aufgebaut, Positiv-/Negativ-Selbsttest gruen,
  alle drei Dateien balanciert. Kontroll-Diff gegen die Repo-Originale: nur die
  beabsichtigten Zeilen.

### Offen / als Naechstes
- **E-04** (OneCalendar erscheint nicht im Teilen-Menue; `text/calendar`/`.ics`).
- **Kachelung der Auswertung** fuer sehr hohe Rollen - eigener Nachzug.
- **Mehr Blau in der Stimme** (Farbwert) - kleiner optionaler Feinschliff.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
  zuletzt vor v1.0.
...
### Anforderungsdokument
- **v6.15 -> v6.16.** Neuer Block "gegenueber Version 6.15": E-02 gebaut (Feld
  "Nicht Auswerten", globale Liste, Filter auf Tag-Ebene ueber Eintraege,
  Aufgaben, Termine - Weg B). E-02 in Kernkonzept 8 und Feature-Tabelle auf
  gebaut gesetzt.

### Commits (heute)
- `f9397fb` - feat: E-02 Nicht Auswerten - Tags global aus Wochenauswertung ausschliessen (Filter auf Tag-Ebene; Eintraege, Aufgaben, Termine; auch aus Suffixen und Tag-Uebersicht)
- `docs:` - Fortschritt Session 64 v2_22 + Anforderungen v6.16 (dieser Commit; zitiert `f9397fb`)

---

## Session 65 - 16. September 2026 (Doku-Hygiene, Chat: Ideen-Eingang gerade gezogen)

### Charakter
- **Reine Doku-Session im Chat, kein Code.** Aus dem Tagesbetrieb kam eine neue
  Idee (Autosave). Beim Aufnehmen fiel auf, dass der Ideen-Eingang hinterherhing;
  Ergebnis ist ein docs-Commit, der ihn gerade zieht. Kein feat, keine
  Anforderungs-Aenderung.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. Stand ueber frischen Clone verifiziert: HEAD war
  `4176b64` (docs S64), Kette lueckenlos bis feat `f9397fb`. Anforderungen v6.16
  und Fortschritt v2_22 aktuell - der verbindliche Stand (Code + settled record)
  stimmt.

### Befund (Ehrlichkeit / Discrepancy Surfacing)
- Der **Ideen-Eingang** hing im Repo selbst hinterher - Byte-Vergleich Repo gegen
  Projekt-Upload: identisch, also nicht nur der Upload:
  - **E-03** stand noch unter "Offene Eingaenge", obwohl in **S61 gebaut** (feat
    `d031935`). Haette nach "Einsortiert" gehoert.
  - **E-04** (OneCalendar/Teilen-Menue) wurde direkt in Warteschlange und
    Anforderungen erfasst, lief nie ueber den Eingang - daher die Luecke in der
    Nummerierung.
  - Ursache: In S64 wurde der Eingang nur halb nachgezogen (nur die
    E-02-gebaut-Zeile kam rein).

### Erledigt (ein docs-Commit, `7c620bb`)
- **Ideen-Eingang gerade gezogen** (`docs/disponere_ideen_eingang_v1_0.md`, in
  place ueberschrieben) per gezielten Splices aus dem Repo-Original,
  Ein-Treffer-Pruefung je Aenderung; Kontroll-Diff = nur die gewollten Zeilen:
  1. **E-03 -> "Einsortiert"** (gebaut S61, feat `d031935`), im Stil E-01/E-02.
  2. **E-03 raus** aus "Offene Eingaenge".
  3. **E-05 - Autosave** neu in "Offene Eingaenge", Status OFFEN.
  4. **E-04** als Pointer-Zeile unter "Einsortiert" - reversibel: auf Wunsch
     spaeter als voller offener Eingang mit Originaltext.
  - Stand-Zeile des Eingangs auf 15.09.2026 gesetzt.

### Neue Idee aufgenommen: E-05 (Autosave)
- **Problem (aus dem Tagesbetrieb):** Ungespeicherter Tipptext geht verloren,
  wenn Android die App im Hintergrund beendet - passiert bei "Neuer Eintrag" und
  beim Event->Eintrag-Bruecken-Sheet (fuer Steffen dasselbe). Nur Tipptext-
  Flaechen betroffen, kein natives FreeScript-Feld, keine Tinte.
- **Vorschlag (noch OFFEN):** Entwurfs-Slot in `shared_preferences`, laufend
  entprellt schreiben + garantiert beim Lebenszyklus-`paused` flushen; beim
  Oeffnen vorbelegen; beim Speichern/Verwerfen leeren.
- **Zu klaeren vor dem Bauen:** (1) Wiederherstellung still oder mit Hinweis
  "Entwurf wiederhergestellt"; (2) `shared_preferences` (ein Slot) vs.
  SQLite-Tabelle; (3) Start-Reihenfolge nach Prefs/DB-Init - Beruehrungspunkt
  mit N5 (Kaltstart-Race).
- Naechster Schritt: Triage im Chat, danach eigene kleine Coding-Session.

### Anforderungsdokument
- **Unveraendert bei v6.16.** Diese Session hat nichts entschieden, was in den
  settled record gehoert - E-05 ist bewusst noch OFFEN. Kein Versions-Bump, um
  keine byte-nahe Leerversion zu erzeugen.

### Offen / als Naechstes
- **E-04** (OneCalendar erscheint nicht im Teilen-Menue; `text/calendar`/`.ics`).
- **E-05** (Autosave) - Triage offen, dann Coding.
- **Kachelung der Auswertung** fuer sehr hohe Rollen - eigener Nachzug.
- **Mehr Blau in der Stimme** (Farbwert) - kleiner optionaler Feinschliff.
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
...
### Commits (heute)
- `7c620bb` - docs: Ideen-Eingang gerade gezogen - E-03 nach Einsortiert (gebaut
  S61), E-04 als Pointer, E-05 Autosave neu aufgenommen

---

## Session 66 - 24. September 2026 (Triage, Chat: E-06 Text markieren entschieden; reine Doku-Session)

### Charakter
- **Reine Triage-Session im Chat, kein Code.** Aus dem Tagesbetrieb (Screenshot
  der Tag-Ansicht #Comos): Gespeicherter Text liess sich nirgends kopieren; der
  Behelf Eintrag -> Aufgabe -> Eintrag war aufwendig und verfaelschte
  Zeitstempel und Verknuepfungen. Ergebnis: ein docs-Commit (`40bde20`).
- *Dieser Block wird in Session 67 nachgetragen* - S66 hatte keinen eigenen
  Fortschritts-Block (v2_23 endete bei S65).

### Entschieden (E-06, Weg 2)
- **Ausschnitt markieren** statt "ganzen Eintrag kopieren" (Standard-Android-
  Auswahl: lange druecken -> Griffe -> Kopieren).
- **Ueberall**, wo gespeicherter Text steht; Tinte und Bilder ausgenommen.
- **Weg 2:** lange druecken wird einheitlich zur Markier-Geste, die bisherigen
  Long-Press-Menues wandern hinter ein Drei-Punkte-Symbol (vertikal) an der Karte. Verworfen: Weg 1
  ("Text markieren" als Menuepunkt mit Leseansicht) - ein Schritt mehr bei
  jedem Kopieren.
- Reihenfolge: E-06 als naechste kurze Session vor E-05 und E-04.

### Commits
- `40bde20` - docs: E-06 Text markieren entschieden (Weg 2) - Anforderungen
  v6.17, Ideen-Eingang

---

## Session 67 - 24. September 2026 (Coding, Chat: E-06 Text ueberall markier- und kopierbar)

### Charakter
- **Coding-Session im Chat-Modus.** E-06 nach der Entscheidung aus S66 gebaut.
  Ein Feat-Commit (zehn Dateien, eine neu), danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. Frischer Clone: HEAD `40bde20`, Kette lueckenlos
  bis feat `f9397fb`. Anforderungen v6.17 und Ideen-Eingang im Projekt
  byte-identisch mit dem Repo.
- **Befunde:** (1) S66 hatte keinen Fortschritts-Block - hier nachgetragen.
  (2) Im Projekt lagen wieder Altversionen (Anforderungen v6_15, Fortschritt
  v2_21/v2_22). (3) `disponere_naechste_session_plan.md` (Stand 01.08.) ist
  abgearbeitet und irrefuehrend. (2) und (3): Aufraeumen nach dieser Session.

### Vor dem Bauen im Code geprueft (Pflicht aus v6.17)
- Alle `onLongPress`-Belegungen eingesammelt - **vier** statt der drei
  dokumentierten: Eintrag -> Aktionsblatt (`entry_card`), Aufgabe -> Bruecke
  (`task_card`, Journal + Heute-Panel), Termin -> Aktionen (`event_card`) und
  zusaetzlich die eigene Karte der **Aufgabenuebersicht** (`_OverviewTaskCard`,
  Bruecke Aufgabe -> Eintrag). Alle vier hinter das Drei-Punkte-Symbol gezogen.
- Wochenauswertung nutzt schon `SelectableText` - dort nichts zu tun
  (Punkt 3 aus v6.17 erledigt).

### Erledigt (zehn Dateien)
- **`lib/widgets/card_menu_button.dart`** (neu): gemeinsames Drei-Punkte-Symbol (vertikal) - dezent
  (Platzhalter-Grau, 18 px), Trefferflaeche 32x32, in
  `SelectionContainer.disabled`, damit es nie Teil einer Auswahl wird.
- **Markieren (`SelectionArea`)**, bildschirmweit je Screen: Journal-Body
  (`journal_screen`), Heute-Panel-Liste (`today_panel`), Tag-Ansicht
  (`tag_view_screen`), Suchergebnisse (`search_screen`, Suchfeld aussen vor),
  Aufgabenuebersicht (`task_overview_screen`, Sortier-Schalter per
  `SelectionContainer.disabled` ausgenommen).
- **Drei-Punkte-Symbol statt langem Druecken** an `EntryCard` (per Stack oben rechts im
  Innenabstand, Layout unveraendert), `TaskCard`, `EventCard`,
  `_OverviewTaskCard` (je rechts am Zeilenende). Das Drei-Punkte-Symbol erscheint nur dort, wo
  vorher lange druecken etwas tat (keins an vergangenen oder erledigten
  Aufgaben). Bei Aufgaben oeffnet das Drei-Punkte-Symbol die Bruecke direkt - wie vorher das
  lange Druecken, inhaltlich unveraendert.
- **Umbenennung:** Callbacks `onLongPress...` -> `onMenu...` durchgaengig
  (Karten, `PastDayView`, `TodayPanel`, Aufrufer in `journal_screen`).
  Kommentare zur alten Geste nachgezogen.

### Umkehrbare Entscheidungen (markiert)
- `SelectionArea` bildschirmweit (Auswahl auch ueber Kartengrenzen) - der
  bevorzugte Weg aus v6.17; Rueckfall `SelectableText` pro Karte war nicht noetig.
- Leer-Einladung "Tippen oder mit dem Stift schreiben ..." und Sortier-Schalter
  nicht markierbar (kein gespeicherter Text).
- `EntryCard` jetzt immer volle Breite (`StackFit.passthrough` +
  `width: double.infinity`): In den Tag-Clustern vergangener Tage war die Karte
  bisher nur inhaltsbreit (unsichtbar auf Papierweiss); so sitzt das Drei-Punkte-Symbol immer am
  rechten Rand und nie ueber der Uhrzeit. Nebenwirkung: Antippen rechts neben
  kurzem Text oeffnet dort jetzt auch Bearbeiten.

### Auf dem MatePad bestaetigt
- Steffen: "Hat hervorragend funktioniert". Byte-Vergleich aller zehn Dateien
  gegen die Auslieferung nach dem Push (frischer Clone): identisch.

### Werkzeug
- Klammer-Pruefer (Mode-Stack) neu aufgebaut, Positiv-/Negativ-Selbsttest
  gruen, alle 50 Repo-Dateien vorab balanciert, alle zehn Dateien danach.
  Aenderungen per Ein-Treffer-Splices (`assert count == 1`); Einrueckung der
  umschlossenen Bloecke nachgezogen (kein `dart format` im Container).

### Anforderungsdokument
- **v6.17 -> v6.18.** Neuer Block "gegenueber Version 6.17": E-06 gebaut.
  Abschnitt "Text markieren & kopieren" und Feature-Zeile auf gebaut; die
  Feature-Zeilen der Bruecken (Eintrag <-> Aufgabe, Termin -> Eintrag) nennen
  jetzt das Drei-Punkte-Symbol statt Long-Press. Ideen-Eingang: E-06 als gebaut markiert.

### Offen / als Naechstes
- **E-05** (Autosave) - Triage der drei offenen Fragen, dann Coding.
- **E-04** (OneCalendar erscheint nicht im Teilen-Menue; `text/calendar`/`.ics`).
- **Doku-Aufraeumen im Projekt:** Altversionen raus, `naechste_session_plan`
  raus.
- **Kachelung der Auswertung** fuer sehr hohe Rollen; **mehr Blau in der
  Stimme** (optional).
- **Unveraendert:** N2 (`huawei_ml_text` raus), N4 (Tag-Ansicht Starttag-only),
  N5 (Kaltstart-Race), Trivia T1-T3, F4/F5/F6; Fable-5-Zweitmeinung bewusst
  zuletzt vor v1.0.
...
### Commits (heute)
- `0ac7dab` - feat: E-06 Text ueberall markier- und kopierbar - SelectionArea in
  Journal, Heute-Panel, Tag-Ansicht, Suche, Aufgabenuebersicht;
  Long-Press-Menues hinter neues Kartensymbol (CardMenuButton)
- `docs:` - Fortschritt Sessions 66+67 v2_24 + Anforderungen v6.18 +
  Ideen-Eingang (dieser Commit; zitiert `0ac7dab`)

---

## Session 68 - 24. September 2026 (Triage + Coding, Chat: E-05 Autosave / Entwurf sichern)

### Charakter
- **Triage und Coding in einer Session.** E-05 zuerst triagiert (drei offene
  Fragen aus dem Ideen-Eingang plus zwei beim Lesen des Codes gefundene), dann
  gebaut. Ein Feat-Commit (vier Dateien, zwei neu), danach dieser Block.

### Geerdet (Session-Ritual)
- Fundament aus Drive gelesen. Frischer Clone: HEAD `0ac7dab` (feat E-06) -
  **Befund:** der zugehoerige docs-Commit fehlte auf GitHub. Die Dateien
  (Fortschritt v2_24, Anforderungen v6.18, Ideen-Eingang) lagen fertig auf
  Vega, waren aber weder committet noch gepusht. Nachgeholt als `25f0db1`,
  Kette per frischem Clone geprueft.

### Triage E-05 (entschieden, alle umkehrbar)
1. **Wiederherstellung mit Hinweis**, nicht still: Zeile "Entwurf von HH:MM
   wiederhergestellt - Verwerfen". Grund: die Sheets haben keinen
   Abbrechen-Knopf - ohne "Verwerfen" gaebe es keinen bewussten Weg, einen
   alten Entwurf loszuwerden.
2. **`shared_preferences`, zwei Slots** statt SQLite-Tabelle: `draft_new_entry`
   und `draft_event_bridge`. Der Bruecken-Entwurf traegt eine Quellen-Kennung
   (Kalender + Termin + Tag) und kommt nur beim **selben** Termin zurueck.
3. **N5 unberuehrt:** gelesen wird erst beim Oeffnen eines Sheets, lange nach
   `_init()`.
4. **Wegwischen behaelt den Entwurf** (neu gefunden). Nur Speichern oder
   "Verwerfen" loescht. Deckt beide denkbaren Verlustursachen ab
   (Prozess beendet / Sheet versehentlich zu).
5. **Gesichert: eigener Text + Tag-Feld** (neu gefunden). Nicht: Bild,
   Anzeige-Tag, Termintext (entsteht neu aus dem Event). **Nicht betroffen:**
   "Eintrag bearbeiten" und das Teilen-Landeblatt (Umfang wie am 15.09.
   vereinbart).

### Erledigt (vier Dateien)
- **`lib/services/draft_store.dart`** (neu): `DraftStore` (Slots, Laden mit
  Quellen-Pruefung, alle Schreib-/Loeschvorgaenge ueber eine Future-Kette,
  damit ein spaetes Schreiben nie ein Loeschen ueberholt), `Draft`
  (JSON: Text, Tags, Zeitpunkt, Quelle), `DraftSession` (haengt an Text- und
  Tag-Controller, schreibt 800 ms entprellt; `flush`, `discardRestored`,
  `clear` nach Speichern, `close` beim Schliessen). Ein Entwurf ohne eigenen
  Text gilt als leer (reines Tag-Feld wird nicht wiederhergestellt).
- **`lib/widgets/draft_hint.dart`** (neu): gemeinsame Hinweiszeile mit
  "Verwerfen" (heute nur Uhrzeit, sonst "TT.MM., HH:MM").
- **`journal_screen.dart`:** `_openEntrySheet` jetzt `Future<void>`/async;
  Entwurf nur beim schlichten neuen Eintrag (kein Bestand, kein geteilter
  Inhalt). `close()` nach dem Sheet, `clear()` in `_saveEntryFromSheet`.
  `didChangeAppLifecycleState`: jeder Zustand ausser `resumed` ruft
  `DraftStore.flushActive()`.
- **`bridge_sheet.dart`:** `showEventToEntrySheet` async, laedt den Entwurf zur
  Quellen-Kennung; `_showBridgeSheet` nimmt optional Tag-Controller und
  `DraftSession` (Hinweis, `clear()` beim Speichern, `close()` am Ende).
  Die beiden anderen Bruecken-Beine unveraendert.

### Umkehrbare Details (markiert)
- Stift-Knopf im "Neuer Eintrag"-Sheet schliesst das Sheet -> vorher getippter
  Text bleibt als Entwurf liegen und kommt beim naechsten "Neuer Eintrag"
  zurueck (bisher war er weg). Folgt der Regel "nur Speichern/Verwerfen
  loescht"; Steffen informiert.
- Entprellung 800 ms; Flush zusaetzlich bei `inactive`/`hidden`/`paused`.

### Auf dem MatePad bestaetigt
- Steffen: "Funktioniert alles." Getestet: Wegwischen/Wiederherstellen,
  Verwerfen, Speichern loescht, App aus der Uebersicht beendet -> Text
  wieder da, Bruecke gleicher/anderer Termin, Bearbeiten/Teilen unveraendert.
- Byte-Vergleich aller vier Dateien gegen die Auslieferung nach dem Push
  (frischer Clone): identisch.

### Werkzeug
- Klammer-Pruefer (Mode-Stack) neu aufgebaut, 6/6 Positiv- und 6/6
  Negativtests gruen, alle 51 Repo-Dateien vorab balanciert, alle vier Dateien
  danach. Aenderungen per Ein-Treffer-Splices (`assert count == 1`). Kein
  Flutter im Container - erste harte Pruefung war der Analyzer auf Vega.

### Anforderungsdokument
- **v6.18 -> v6.19.** Neuer Block "gegenueber Version 6.18": E-05 triagiert
  und gebaut. Neuer Funktions-Abschnitt "Entwurf sichern" und Feature-Zeile.
  Ideen-Eingang: E-05 von "Offene Eingaenge" nach "Einsortiert", als gebaut
  markiert.
...
### Commits (heute)
- `25f0db1` - docs: Fortschritt v2_24 + Anforderungen v6.18 + Ideen-Eingang
  (E-06; nachgeholter Push aus Session 67)
- `5783e20` - feat: E-05 Autosave - Entwurf in Neuer-Eintrag- und
  Termin-Bruecken-Sheet (shared_preferences, zwei Slots, Hinweis mit
  Verwerfen, Flush bei Hintergrund)
- `docs:` - Fortschritt Session 68 v2_25 + Anforderungen v6.19 +
  Ideen-Eingang (dieser Commit; zitiert `5783e20`)
